...
def fix_python_docstrings():
    ...
    if retriever_types_file.exists():
        ...
        if content != original:
            ...
            print(f"Fixed {retriever_types_file.name}")
    ...
    if retriever_file.exists():
        ...
        if content != original:
            ...
            print(f"Fixed {retriever_file.name}")
...
if __name__ == "__main__":
    ...
    print(f"\nFixed {len(fixed_rst)} RST files")
    print("Run 'poetry run nox -s docs_fast' to build without treating warnings as errors")