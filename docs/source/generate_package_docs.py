    ...
    "haive-core": {
        ...
        "modules": {
            "engine": {
                ...
                "icon": "🤖",
                "submodules": ["base", "aug_llm", "document", "agent", "embedding", "retriever", "vectorstore", "tool"]
            },
            "schema": {
                "title": "Schema System", 
                "description": "Dynamic state schema composition",
                "icon": "📊",
                "submodules": ["state_schema", "schema_composer", "compatibility", "prebuilt"]
            },
            "graph": {
                "title": "Graph System",
                "description": "State machines and workflow orchestration", 
                "icon": "🌐",
                "submodules": ["state_graph", "node", "patterns", "routers", "branches", "utils"]
            },
            "persistence": {
                "title": "Persistence",
    ...
    "haive-agents": {
        ...
        "modules": {
            ...
            "conversation": {
                ...
                "icon": "🗣️",
                "submodules": ["base", "collaborative", "debate", "directed", "round_robin", "social_media"]
            },
            "rag": {
                "title": "RAG Agents",
                "description": "Retrieval-augmented generation",
                "icon": "📚",
                "submodules": ["base", "adaptive_rag", "self_rag", "multi_strategy", "hyde", "db_rag"]
            },
            "react": {
                "title": "ReAct Agents",
            ...
            "reasoning_and_critique": {
                ...
                "icon": "🧠",
                "submodules": ["lats", "reflection", "reflexion", "tot", "mcts", "self_discover"]
            },
            "multi": {
                "title": "Multi-Agent",
...
def create_module_index(package_name: str, module_name: str, module_info: Dict, package_prefix: str) -> str:
...
{"".join(grid_items)}"""
    ...
    # Create toctree
    toctree_entries = [f"   {module}/index" for module in package_info["modules"].keys()]
...
def create_submodule_index(package_name: str, module_name: str, submodule_name: str, package_prefix: str) -> str:
...
def generate_docs():
    ...
    for package_name, package_info in PACKAGE_INFO.items():
        ...
        for module_name, module_info in package_info["modules"].items():
            ...
            # Create module index
            module_index = create_module_index(package_name, module_name, module_info, package_prefix)
            ...
            print(f"Created {module_dir}/module.rst")
            ...
            for submodule_name in module_info.get("submodules", []):
                ...
                submodule_index = create_submodule_index(package_name, module_name, submodule_name, package_prefix)
                (submodule_dir / "index.rst").write_text(submodule_index)
                print(f"Created {submodule_dir}/index.rst")


if __name__ == "__main__":
    print("Generating Haive package documentation structure...")
    generate_docs()
    print("\nDone! Documentation structure created.")
    print("\nNext steps:")
    print("1. Run: poetry run sphinx-build -b html docs/source docs/build/html")
    print("2. View at: http://localhost:8002/api/{package-name}/index.html")