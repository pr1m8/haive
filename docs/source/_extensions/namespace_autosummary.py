...
from sphinx.ext.autosummary import Autosummary, import_by_name, ImportExceptionGroup
from sphinx.ext.autosummary.generate import AutosummaryRenderer, find_autosummary_in_docstring
...
def safe_import_by_name(name: str, prefixes: List[Optional[str]] = [None]) -> Tuple[str, Any, Any, str]:
    """
    ...
    try:
        ...
        try:
            ...
            return name.split('.')[-1], obj, '.'.join(parts[:-1]) if len(parts) > 1 else None, name
        except Exception:
            pass
        ...
        return parts[-1], DummyModule(), '.'.join(parts[:-1]) if len(parts) > 1 else None, name
...
def process_namespace_packages(app, what, name, obj, options, lines):
    ...
    if what == 'module' and hasattr(obj, '__path__'):
        ...
        lines.insert(0, f"**Namespace Package**")
        lines.insert(1, "")
