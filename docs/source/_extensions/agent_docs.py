"""Sphinx extension for automatic agent documentation with examples and visualization."""
...
class AgentDocDirective(SphinxDirective):
    ...
    def _create_example_section(self, agent_name: str, metadata: Dict[str, Any]) -> Optional[nodes.section]:
    ...
    def _create_visualization_section(self, agent_name: str, metadata: Dict[str, Any]) -> Optional[nodes.section]:
        ...
        # Graph type info
        para = nodes.paragraph()
        para += nodes.Text(f"This agent supports graph visualization (type: {viz_info.get('graph_type', 'simple')})")
        section += para
...
"""
    ...
    def _create_config_section(self, agent_name: str, metadata: Dict[str, Any]) -> Optional[nodes.section]:
        ...
        if metadata.get('state_schema'):
            para = nodes.paragraph()
            para += nodes.Text(f"State Schema: ")
