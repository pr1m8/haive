...
This extension recognizes the consistent patterns in haive-games and creates
better documentation that respects the structure and nuances of each game.
"""
...
class GameInfo:
    ...
    def _scan_components(self) -> Dict[str, bool]:
        ...
        standard_files = [
            "agent.py", "config.py", "state.py", "models.py", 
            "engines.py", "example.py", "ui.py", "utils.py"
        ]
...
class GameCategorizer:
    ...
    GAME_PATTERNS = {
        'board_games': [
            ...
            'tic_tac_toe', 'tictactoe', 'connect4', 'mancala',
            'fox_and_geese', 'backgammon'
        ],
        'card_games': [
            'poker', 'hold_em', 'holdem', 'blackjack', 'bs', 
            'uno', 'solitaire', 'hearts', 'spades', 'bridge'
        ],
        'single_player': [
            'wordle', 'sudoku', '2048', 'twenty_fourty_eight',
            'minesweeper', 'mine_sweeper', 'flow_free', 'rubiks',
            'towers_of_hanoi', 'crossword', 'word_search', 'solitaire'
        ],
        'social_deduction': [
            'mafia', 'among_us', 'clue', 'cluedo', 'werewolf',
            'secret_hitler', 'resistance'
        ],
        'strategy_games': [
            'risk', 'monopoly', 'dominoes', 'battleship',
            'civilization', 'settlers', 'catan'
        ],
        'arcade_games': [
            'pacman', 'tetris', 'snake', 'pong', 'asteroids',
            'space_invaders', 'breakout'
        ]
    }
...
class GameDocumentationGenerator:
    ...
    def _scan_games(self):
        ...
        for item in self.games_path.iterdir():
            if item.is_dir() and not item.name.startswith(('_', '.')):
                ...
                if item.name in ['core', 'framework', 'base', 'base_v2', 'common', 'utils', 'api']:
                    continue
        ...
        if any(g.has_example for g in sorted_games):
            ...
            for game in sorted_games[:3]:  # Show top 3 examples
                if game.has_example:
                    ...
                    lines.append("")
                    lines.append(f"   from haive.games.{game.name} import {game.title.replace(' ', '')}Agent")
                    lines.append(f"   ")
                    lines.append(f"   agent = {game.title.replace(' ', '')}Agent()")
                    lines.append(f"   result = agent.run()")
                    lines.append("")
    ...
    def generate_main_index(self) -> str:
        ...
        lines.append(".. note::")
        lines.append("")
        lines.append(f"   **{len(all_games)} games available** across {len([c for c in self.categories.values() if c.games])} categories")
        ...
        if excellent_games:
            ...
            lines.append("----------------")
            lines.append("")
            lines.append("These games have comprehensive documentation, examples, and full feature sets:")
            lines.append("")
...
class GamesAutoDocDirective(SphinxDirective):
    ...
    def run(self):
        ...
        if 'games-path' in self.options:
            ...
            games_path = Path(env.srcdir).parent.parent / "packages" / "haive-games" / "src" / "haive" / "games"
