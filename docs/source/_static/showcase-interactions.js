...
(function () {
  ...
  function initializeShowcaseTabs() {
    ...
    tabGroups.forEach((tabGroup) => {
      ...
      const contents =
        tabGroup.parentElement.querySelectorAll(".showcase-content");
  ...
  function initializeCardInteractions() {
    const cards = document.querySelectorAll(
      ".showcase-card, .agent-card, .game-card",
    );
    ...
    cards.forEach((card) => {
      ...
      card.addEventListener("click", (e) => {
        ...
        const cardTitle =
          card.querySelector(".agent-title, .game-title")?.textContent ||
          "Unknown";
  ...
  function initializeShowcaseSearch() {
    ...
    const showcaseContainer = document.querySelector(
      ".agent-showcase, .games-showcase",
    );
    ...
    showcaseContainer.parentElement.insertBefore(
      searchContainer,
      showcaseContainer,
    );
    ...
    const performSearch = debounce((query) => {
      ...
      cards.forEach((card) => {
        const title =
          card.querySelector(".agent-title, .game-title")?.textContent || "";
        const description =
          card.querySelector(".agent-description, .game-description")
            ?.textContent || "";
        const features = Array.from(
          card.querySelectorAll(".agent-features li, .game-feature"),
        )
          .map((el) => el.textContent)
          .join(" ");

        const searchContent =
          `${title} ${description} ${features}`.toLowerCase();
