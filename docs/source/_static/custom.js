...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.game-screenshot, img[alt*="UI"], img[alt*="Graph"]').forEach(img => {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
            e.preventDefault();
            const searchInput = document.querySelector('.sidebar-search-container input[type="search"]');
            if (searchInput) {
                searchInput.focus();
            }
        }
    ...
    const currentLinks = document.querySelectorAll('.sidebar-tree .reference.current');
    currentLinks.forEach(currentLink => {
        let parent = currentLink.parentNode;
        while (parent && parent.classList) {
            if (parent.classList.contains('toctree-l1') || 
    ...
    const apiSections = document.querySelectorAll('.py.class, .py.function, .py.method');
...
console.log('%c🤖 Welcome to Haive Documentation! 🤖', 
    'font-size: 20px; font-weight: bold; color: #2962ff;');
console.log('%cBuild amazing AI agents with Haive!', 
    'font-size: 14px; color: #4fc3f7;');
console.log('%cVisit our GitHub: https://github.com/will-astley/haive', 
