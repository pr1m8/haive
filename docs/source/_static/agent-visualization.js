...
class AgentGraphVisualizer {
    ...
    assignLevels(nodes, edges) {
        ...
        while (hasChanges) {
            ...
            edges.forEach(edge => {
                if (nodeLevel[edge.source] === currentLevel - 1 && !nodeLevel.hasOwnProperty(edge.target)) {
    ...
    renderNode(node, layout) {
        ...
        text.setAttribute('class', `node-text ${['start', 'end', 'tool', 'agent'].includes(node.type) ? 'light-text' : ''}`);
    ...
    highlightStep(stepIndex) {
        ...
        if (step.fromNodeId) {
            const edge = this.svg.querySelector(`[data-edge-id="${step.fromNodeId}-${step.nodeId}"]`);
            if (edge) {
...
class StateHistoryVisualizer {
    ...
    setCurrentState(step) {
        ...
        if (targetItem) {
            ...
            // Update indicator
            const indicator = this.container.querySelector('.current-state-indicator');
            if (indicator) {
...
class ExecutionTraceVisualizer {
    ...
    renderTraceSteps() {
        return this.traceData.map((step, index) => `
    ...
    calculateTotalTime() {
        return this.traceData.reduce((total, step) => total + (step.duration || 0), 0);
    }
...
function initializeAgentVisualizations() {
    ...
    document.querySelectorAll('[data-state-history]').forEach(container => {
        const stateHistory = JSON.parse(container.getAttribute('data-state-history'));
    ...
    document.querySelectorAll('[data-execution-trace]').forEach(container => {
        const traceData = JSON.parse(container.getAttribute('data-execution-trace'));
