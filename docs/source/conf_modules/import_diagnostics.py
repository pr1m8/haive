...
def find_all_python_modules(search_paths: List[str], base_dir: str | None = None) -> Set[str]:
...
def diagnose_imports(autoapi_dirs: List[str], base_dir: str | None = None) -> Tuple[Dict, Set[str]]:
...
def save_import_diagnosis(results: Dict, mock_imports: Set[str], output_dir: str = "docs/logs"):
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"## Modules to Mock\n")
        for module in sorted(mock_imports):
            f.write(f"- `{module}`\n")
        
        f.write(f"\n## Failed Imports\n")
        for result in results["missing_module"] + results["import_error"]:
            f.write(f"- **{result['module']}**: {result['error_type']} - {result['error']}\n")
    
    logger.info(f"📁 Diagnosis results saved to:")
...
def get_autodoc_mock_imports_from_diagnosis(autoapi_dirs: List[str], base_dir: str | None = None) -> List[str]:
    ...
    try:
        ...
        return sorted(list(mock_imports))
    except Exception as e:
        logger.error(f"Failed to diagnose imports: {e}")
        # Return some known problematic imports as fallback
        return ['pokebase', 'agents', 'haive.core.exceptions', 'haive.core.graph.state_graph.compiled_state_graph']