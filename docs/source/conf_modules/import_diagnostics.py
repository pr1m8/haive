    ...
    if fast_mode or len(all_modules) > sample_limit:
        ...
        for module in all_modules:
            ...
            if ("." not in module or module.count(".") <= 2
                    or any(x in module
                           for x in ["examples", "test", "demo", "archive"])):
                sample_modules.add(module)

        if len(sample_modules) < len(all_modules):
            mode_reason = ("fast mode enabled" if fast_mode else
                           f"module count exceeds {sample_limit}")
            logger.info(
    ...
    for result in results["missing_module"] + results["import_error"]:
        ...
        if "No module named" in error_msg:
            ...
            missing_module = (error_msg.split("'", )[1]
                              if "'" in error_msg else error_msg.split()[-1])
