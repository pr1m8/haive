...
.. raw:: html
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize graph
        const graphData = JSON.parse(document.getElementById('reactwithmemory-graph').dataset.agentGraph);
        new AgentGraphVisualizer('reactwithmemory-graph', graphData);
        
        // Initialize state history
        const stateData = JSON.parse(document.getElementById('reactwithmemory-state-history').dataset.stateHistory);
        new StateHistoryVisualizer('reactwithmemory-state-history', stateData);
...
.. code-block:: python
    ...
    agent = ReactWithMemoryAgent(
    name="reactwithmemory",
    model="gpt-4",
    temperature=0.7
    )

    # Run the agent
    result = await agent.arun("Example task for ReactWithMemoryAgent")
    print(result)
