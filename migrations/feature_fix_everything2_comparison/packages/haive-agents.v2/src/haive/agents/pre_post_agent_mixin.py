...
class PrePostAgentMixin:
    ...
    async def run_with_pre_post_processing(self,
        ...
        if not hasattr(self,
                       '_pre_transformer') or self._pre_transformer is None:
        ...
        if self.pre_agent and not (self.skip_pre_if_empty and not input_data):
            logger.debug(
                f"Running pre-processing agent: {self.pre_agent.name}")

            # Execute pre-processing hooks
            if hasattr(self, 'execute_hooks'):
                self.execute_hooks(
                    (HookEvent.BEFORE_GRADING if 'grading' in getattr(
                        self.pre_agent, 'name', '') else
            ...
            if hasattr(self, 'execute_hooks'):
                self.execute_hooks(
                    (HookEvent.AFTER_GRADING if 'grading' in getattr(
                        self.pre_agent, 'name', '') else HookEvent.AFTER_NODE),
        ...
        main_result = (await super().arun(current_input) if hasattr(
            super(), 'arun') else current_input)

        # Stage 3: Post-processing (optional)
        post_result = None
        final_result = main_result

        if self.post_agent and not (self.skip_post_if_empty
                                    and not main_result):
            logger.debug(
                f"Running post-processing agent: {self.post_agent.name}")
            ...
            if hasattr(self, 'execute_hooks'):
                self.execute_hooks(
                    (HookEvent.BEFORE_REFLECTION if 'reflection' in getattr(
                        self.post_agent, 'name', '') else
                     HookEvent.BEFORE_NODE),
            ...
            if hasattr(self, 'execute_hooks'):
                self.execute_hooks(
                    (HookEvent.AFTER_REFLECTION if 'reflection' in getattr(
                        self.post_agent, 'name', '') else
                     HookEvent.AFTER_NODE),
        ...
        if self.combine_results:
            ...
            if (self.preserve_original and isinstance(main_result, dict)
            ...
            if self.use_pre_transform or self.use_post_transform:
                combined_result['transformations_applied'] = {
                    'pre_transform': (self.pre_transform_type
                                      if self.use_pre_transform else None),
                    'post_transform': (self.post_transform_type
                                       if self.use_post_transform else None),
                }
    ...
    async def arun(self, input_data: Any) -> Any:
        ...
        # Standard execution
        return await super().arun(input_data) if hasattr(
            super(), 'arun') else input_data
