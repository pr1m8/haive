...
from migrations.feature_fix_everything2_comparison.packages.haive-agents.v2.src.haive.agents.memory_v2.memory_v2.memory_state_original import (
    EnhancedMemoryItem, )
from migrations.feature_fix_everything2_comparison.packages.haive-agents.v2.src.haive.agents.memory_v2.memory_v2.message_document_converter import MessageDocumentConverter
...
class KGMemoryConfig(BaseModel):
    ...
    memory_node_types: list[str] = Field(default=[
        'Person',
        'Organization',
        'Location',
        'Concept',
    ...
    memory_relationships: list[str | tuple[str, str, str]] = Field(default=[
    ...
    storage_backend: GraphStorageBackend = Field(
        default=GraphStorageBackend.MEMORY)
...
class GraphDatabaseConnector:
    ...
    async def connect(self) -> None:
        ...
        if self.backend == GraphStorageBackend.NEO4J:
            ...
            logger.warning(
                f"Backend {self.backend} not implemented, using memory")
            self._connection = {'type': 'memory', 'graphs': {}}
    ...
    async def _connect_neo4j(self) -> None:
        ...
        try:
            ...
            async with driver.session(
                    database=self.config.neo4j_database) as session:
    ...
    async def store_knowledge_graph(
        ...
        try:
            ...
            logger.error(
                f"Unknown connection type: {self._connection['type']}")
            return False
    ...
    async def retrieve_graph(
        ...
        try:
            if self._connection['type'] == 'memory':
                ...
                file_path = os.path.join(self._connection['path'],
                                         f"{graph_id}.json")
                if os.path.exists(file_path):
                    ...
                    for rel_data in graph_data['relationships']:
                        rel = EntityRelationship(
                            ...
                            supporting_evidence=rel_data.get(
                                'supporting_evidence'),
                        )
...
class KGMemoryAgent:
    ...
    def __init__(self, config: KGMemoryConfig):
        ...
        logger.info(
            f"Initialized KGMemoryAgent with backend: {config.storage_backend}"
        )
    ...
    async def process_memories_to_graph(
        ...
        try:
            ...
            for doc in graph_docs:
                ...
                for rel in doc.relationships:
                    entity_rel = EntityRelationship.from_graph_relationship(
                        rel,
                        confidence_score=max(self.config.confidence_threshold,
                                             0.8),
                    )
            ...
            metadata = {
                ...
                'memory_types': list({m.memory_type.value
                                      for m in memories}),
                'importance_levels':
                list({m.importance.value
                      for m in memories}),
                'created_at': datetime.now().isoformat(),
    ...
    async def process_conversation_to_graph(
        ...
        for ts_doc in timestamped_docs:
            doc = Document(page_content=ts_doc.page_content,
                           metadata=ts_doc.metadata)
            documents.append(doc)
    ...
    async def retrieve_memory_graph(self,
                                    graph_id: str) -> KnowledgeGraph | None:
    ...
    async def query_graph_by_entity(self,
        ...
        """
        if self.db_connector._connection and self.db_connector._connection[
                'type'] == 'neo4j':
    ...
    async def _query_neo4j_entity(self,
                                  entity_name: str) -> list[dict[str, Any]]:
    ...
    async def _query_memory_entity(self,
                                   entity_name: str) -> list[dict[str, Any]]:
        """Query memory storage for entity relationships."""
        results = []

        for graph_id, graph_data in self.db_connector._connection[
                'graphs'].items():
