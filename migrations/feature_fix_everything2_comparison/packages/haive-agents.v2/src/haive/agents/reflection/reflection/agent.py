...
from migrations.feature_fix_everything2_comparison.packages.haive-agents.v2.src.haive.agents.reflection.reflection.models import ExpertiseConfig, GradingResult, ReflectionConfig
from migrations.feature_fix_everything2_comparison.packages.haive-agents.v2.src.haive.agents.reflection.reflection.prompts import GRADING_SYSTEM_PROMPT, REFLECTION_SYSTEM_PROMPT
...
class PrePostMultiAgent(MultiAgent, Generic[TPreAgent, TMainAgent,
                                            TPostAgent]):
...
class ReflectionAgent(SimpleAgent):
    ...
    reflection_mode: str = Field(
        default="improve",
        description="Mode: improve, critique, or both",
    )
...
def create_reflection_agent(
    ...
    **kwargs,
    ...
    if not engine:
        engine = AugLLMConfig(system_message=REFLECTION_SYSTEM_PROMPT,
                              temperature=0.3)
...
def create_graded_reflection_agent(
    ...
    **kwargs,
    ...
    return GradingAgent(
        name=name,
        engine=AugLLMConfig(system_message=GRADING_SYSTEM_PROMPT,
                            temperature=0.1),
        **kwargs,
    )
