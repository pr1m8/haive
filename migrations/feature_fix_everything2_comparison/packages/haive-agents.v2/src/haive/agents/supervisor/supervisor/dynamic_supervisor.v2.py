...
This module provides a dynamic supervisor agent that can add/remove
agents at runtime, adapt agent responses, and handle complex multi-agent
coordination patterns similar to LangGraph's supervisor package but with
enhanced Haive-specific functionality.
"""
...
class DynamicSupervisorAgent(ReactAgent):
    """Dynamic supervisor agent with runtime agent management and adaptive.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        routing_model = DynamicChoiceModel[str](
            options=[],
            model_name="DynamicSupervisorChoice",
            include_end=True,
        )
    ...
    def _aggregate_agent_tools(self) -> dict:
        """Aggregate tools from all registered agents into supervisor routing.
        ...
        return {
            ...
            "tool_to_agent": tool_to_agent_mapping
        }

    def _create_dynamic_tool_choice(self) -> str:
        """Create dynamic routing choice that includes tools from registered.
    ...
    async def register_agent(
        ...
        if success:
            ...
            console.print(
                f"[green]✅ Registered dynamic agent:[/green] {agent.name}")
    ...
    async def unregister_agent(
        ...
        if success:
            ...
            console.print(
                f"[red]❌ Unregistered dynamic agent:[/red] {agent_name}")
    ...
    def _create_coordinator_node(self) -> Callable:
        ...
        async def coordinator_node(state: DynamicSupervisorState,
                                   config=None) -> dict:
            ...
            return {
                ...
                execution_result,
                "execution_queue":
                ([target_agent] if not self.enable_parallel_execution else
                 [*state.execution_queue, target_agent]),
            }
    ...
    def _create_response_adapter_node(self) -> Callable:
        ...
        async def adapter_node(state: DynamicSupervisorState,
                               config=None) -> dict:
            ...
            adapted_messages = state.adapt_response_for_agent(
                agent_name,
                state.current_execution.messages,
            )
    ...
    def _create_enhanced_agent_wrapper(self, agent_name: str) -> Callable:
        ...
        async def agent_wrapper(state: DynamicSupervisorState,
                                config=None) -> dict:
            ...
            async with self._execution_lock:
                ...
                try:
                    ...
                    result = await asyncio.wait_for(
                        agent.ainvoke(agent_state, config),
    ...
    async def _analyze_input(self,
                             state: DynamicSupervisorState) -> dict[str, Any]:
    ...
    def _get_available_agents_with_context(
        ...
        for agent_name in self.agent_registry.get_available_agents():
            ...
            agents_context[agent_name] = {
                "capability":
                self.agent_registry.get_agent_capability(agent_name),
                "performance":
                performance,
                "priority":
                agent_config.priority if agent_config else 1,
                "available":
                not (agent_config
    ...
    def _create_enhanced_decision_prompt(
        ...
        for agent_name, context in available_agents.items():
            ...
            tools_text = f"Tools: {
                ', '.join(agent_tools)}" if agent_tools else "No tools"
  ...
  Performance: {success_rate:.1f}% success rate, {performance.get("executions", 0)} executions"""
        ...
        agents_text = "\n".join(
            agent_descriptions
        ) if agent_descriptions else "No agents available"
        ...
        if tool_info and tool_info.get("tools"):
            ...
            for tool_name, agent_name in tool_info.get("tool_to_agent",
                                                       {}).items():
...
{context_text}"""
        ...
        for agent_name in available_agents:
            if agent_name.upper() in content_upper:
                return {
                    "target": agent_name,
                    "reasoning":
                    f"Selected {agent_name} based on content match",
                    "confidence": 0.7,
                }
    ...
    def _prepare_enhanced_agent_state(
        self,
        supervisor_state: DynamicSupervisorState,
        agent: Agent,
    ) -> Any:
        ...
        if hasattr(agent, "state_schema") and agent.state_schema:
            try:
                ...
                for field_name in schema_fields:
                    if hasattr(supervisor_state,
                               field_name) and field_name != "messages":
                        state_data[field_name] = getattr(
                            supervisor_state, field_name)

                return agent.state_schema(**state_data)

            except Exception as e:
                logger.warning(
                    f"Could not create specific state for {agent.name}: {e}")
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        for agent_name in state.get_available_agents():
            summary["agent_performance"][
                agent_name] = state.get_agent_performance(agent_name, )
    ...
    def print_supervisor_dashboard(self) -> None:
        ...
        table.add_row("Total Executions",
                      str(len(state.agent_execution_history)))
        ...
        table.add_row("Parallel Execution",
                      str(self.enable_parallel_execution))
            ...
            for decision in recent_decisions:
                decisions_table.add_row(
                    ...
                    (decision.reasoning[:50] + "..."
                     if len(decision.reasoning) > 50 else decision.reasoning),
