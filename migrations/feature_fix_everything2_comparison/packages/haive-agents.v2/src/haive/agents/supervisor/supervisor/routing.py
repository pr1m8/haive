...
class RoutingContext(BaseModel):
    ...
    previous_agent: str | None = Field(None,
                                       description='Previously active agent')
...
class TaskClassifier:
    ...
    TASK_PATTERNS = {
        ...
        'math':
        ['calculate', 'compute', 'solve', 'equation', 'number', 'math'],
        'writing':
        ['write', 'draft', 'compose', 'create', 'generate', 'summarize'],
        'coding':
        ['code', 'program', 'implement', 'debug', 'function', 'script'],
    ...
    @classmethod
    def estimate_complexity(cls, message: str,
...
class LLMRoutingStrategy(BaseRoutingStrategy):
    ...
    async def make_routing_decision(
        ...
        messages = [
            ...
            HumanMessage(
                content=f"Route this request: {context.last_message}"),
        ]
    ...
    def _build_routing_prompt(
        ...
        for agent in available_agents:
            if agent == 'END':
                ...
                capability = agent_capabilities.get(agent,
                                                    f"Handles {agent} tasks")
...
class RuleBasedRoutingStrategy(BaseRoutingStrategy):
    ...
    async def make_routing_decision(
        ...
        if available_agents and available_agents[0] != 'END':
            return RoutingDecision(choice=available_agents[0],
...
class DynamicRoutingEngine:
    ...
    def __init__(
        ...
        if routing_strategy:
            ...
            self.routing_strategy = LLMRoutingStrategy(routing_engine,
                                                       routing_model)
        else:
    ...
    def _extract_context(self, state: Any) -> RoutingContext:
        ...
        # Estimate complexity
        complexity = TaskClassifier.estimate_complexity(
