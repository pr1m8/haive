...
class DynamicExecutorNode:
    ...
    async def __call__(
        ...
        target_agent_name = state_dict.get('target_agent',
                                           state_dict.get('next_agent'))
    ...
    def _prepare_agent_input(self, agent: Any,
        ...
        if hasattr(agent, 'state_schema') and agent.state_schema:
            logger.info(
                f"Using agent's state schema: {agent.state_schema.__name__}")
        ...
        if isinstance(result, dict):
            ...
            for key in result:
                if key == 'messages' and isinstance(result[key], list):
                    logger.info(
                        f"  Updating messages: {len(result[key])} items")
                else:
...
def create_dynamic_executor_node(
        agent_registry: dict[str, Any]) -> DynamicExecutorNode:
