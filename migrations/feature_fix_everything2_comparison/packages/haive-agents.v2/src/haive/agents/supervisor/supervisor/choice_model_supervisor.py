    ...
    def _run(self, agent_type: str, capability_description: str) -> str:
        ...
        agent_templates = {
            'research': {
                'name': 'research_agent',
                'system_message':
                'You are a research specialist. Find and analyze information thoroughly. Use tools when available.',
            ...
            'math': {
                'name': 'math_agent',
                'system_message':
                'You are a mathematician. Solve problems step by step using tools when helpful.',
...
class ChoiceModelSupervisor(ReactAgent):
    ...
    max_agents: int = Field(default=8,
                            description='Maximum agents to maintain')
    ...
    _agent_selection_tool: AgentSelectionTool | None = PrivateAttr(
        default=None)
    ...
    def _create_supervisor_decision_node(self):
        ...
        async def supervisor_node(state: Any) -> dict[str, Any]:
            ...
            try:
                ...
                if selected_agent and selected_agent not in {
                        'CREATE_NEW_AGENT', 'END'
                }:
    ...
    def _create_agent_executor_node(self):
        ...
        async def executor_node(state: Any) -> dict[str, Any]:
            ...
            try:
                ...
                logger.info(
                    f"✅ ReactAgent {target_agent} executed successfully")
                return update
...
if __name__ == '__main__':
    ...
    async def test_choice_model_supervisor():
        ...
        supervisor = ChoiceModelSupervisor(name='choice_supervisof',
                                           max_agents=5)
        ...
        await supervisor.ainvoke(
            {
                'messages': [
                    HumanMessage(
                        content='Write Python code to implement quicksort'),
                ],
            }, )
