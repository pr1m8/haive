...
def should_grade_documents(state: MultiAgentRAGState) -> str:
    ...
    if state.graded_documents:
        relevant_count = sum(1 for doc in state.graded_documents
                             if doc.is_relevant)
    ...
    def _setup_conditional_routing(self):
        ...
        self.add_conditional_edge(
            ...
            destinations={
                ...
                "generate":
                self.answer_agent,
                "refine": (self.query_refiner
                           if self.query_refiner else self.retrieval_agent),
            },
...
class ParallelRAGMultiAgent(ParallelAgent):
    ...
    def __init__(self,
                 rag_agents: list[BaseRAGMultiAgent] | None = None,
                 **kwargs):
...
class AdaptiveRAGMultiAgent(ConditionalAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.complex_rag = complex_rag or IterativeRAGMultiAgent(
            name="Complex RAG")
...
def validate_multi_agent_compatibility(agents: list[Any]) -> dict[str, Any]:
    ...
    return {
        ...
        sum(1 for result in compatibility_results.values()
