...
class SubQuery(BaseModel):
    ...
    dependencies: list[str] = Field(
        description='IDs of sub-queries this depends on')
    priority: int = Field(ge=1,
                          le=10,
                          description='Priority level (1=highest)')

    expected_info: str = Field(
        description='What information this should retrieve')
...
class QueryPlan(BaseModel):
    ...
    query_complexity: QueryComplexity = Field(
        description='Overall query complexity')
    ...
    execution_order: list[str] = Field(
        description='Order to execute sub-queries')
    ...
    estimated_retrievals: int = Field(
        description='Estimated number of retrievals')
    resource_requirements: dict[str,
                                Any] = Field(description='Required resources')

    planning_confidence: float = Field(ge=0.0,
                                       le=1.0,
                                       description='Confidence in plan')
...
class SubQueryResult(BaseModel):
    ...
    relevance_score: float = Field(ge=0.0,
                                   le=1.0,
                                   description='Relevance of results')
...
class QueryPlanningResult(BaseModel):
    ...
    sub_query_results: list[SubQueryResult] = Field(
        description='All sub-query results')
    ...
    synthesis_quality: float = Field(ge=0.0,
                                     le=1.0,
    ...
    execution_metadata: dict[str,
                             Any] = Field(description='Execution metadata')
...
class QueryPlanningRAGAgent(Agent):
    ...
    def create_query_plan(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(
            f"Query plan created: {len(query_plan.sub_queries)} sub-queries")
    ...
    def execute_sub_query(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if current_idx >= len(query_plan.execution_order
            ...
            sub_result = self.execution_engine.invoke(
                {
                    ...
                    'query_type':
                    sub_query.query_type.value,
                    'expected_info':
                    sub_query.expected_info,
                    'success_criteria':
                    sub_query.success_criteria,
                    'dependent_results':
                    str(dependent_results),
                    'documents_summary':
                    f"{len(self.documents)} documents available",
                }, )
    ...
    def synthesize_results(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        execution_stats = {
            ...
            'successful':
            sum(1 for r in sub_results if r.execution_successful),
        ...
        planning_result = self.synthesis_engine.invoke(
            {
                ...
                'query_plan_summary':
                f"Complexity: {query_plan.query_complexity}, Sub-queries: {len(query_plan.sub_queries)}",
    ...
    def build_graph(self) -> BaseGraph:
        ...
        graph.add_conditional_edges(
            ...
            self.should_continue_execution,
            {
                ...
                'synthesize_results':
                'synthesize_results',  # Move to synthesis
            },
        )
