...
class Candidate(BaseModel, Generic[T]):
    ...
    def __str__(self) -> str:
        ...
        content_preview = content_str[:50] + '...' if len(
            content_str) > 50 else content_str
...
class Score(BaseModel):
    ...
    value: float = Field(description='Numerical score between 0 and 1',
                         ge=0,
                         le=1)
    ...
    def __str__(self) -> str:
        ...
        if self.feedback:
            feedback_preview = (self.feedback[:30] + '...'
                                if len(self.feedback) > 30 else self.feedback)
...
class CandidateGeneration(BaseModel, Generic[T]):
    ...
    reasoning: str = Field(
        description='The reasoning behind the generated candidates')
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            'examples': [
                {
                    'reasoning':
                    "I'll explore different approaches to solve this problem...",
                    'candidate_contents': [
    ...
    def to_candidates(self) -> list[Candidate[T]]:
        ...
        return [
            Candidate(content=content) for content in self.candidate_contents
        ]
        ...
        'examples': [
            {
                ...
                0.85,
                'feedback':
                'This solution is good because... However, it could be improved by...',
            },
        ],
    }, )
...
class EquationGeneration(BaseModel):
    ...
    reasoning: str = Field(
        description='The reasoning behind the generated equations')
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            'examples': [
                {
                    'reasoning':
                    "I'll try different combinations of operations...",
                    'equations': [
                        {
                            'tokens': [3.0, 4.0, '+', 8.0, '*']
                        },
