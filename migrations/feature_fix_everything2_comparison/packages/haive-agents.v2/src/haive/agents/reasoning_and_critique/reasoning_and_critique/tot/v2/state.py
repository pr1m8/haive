...
class ToTState(MessagesState):
    ...
    problem_type: str | None = Field(
        default=None,
        description="Type/category of problem",
    )
    problem_context: dict[str, Any] = Field(
        default_factory=dict,
        description="Additional problem context",
    )
    ...
    selected_candidates: list[ScoredCandidate] = Field(
        default_factory=list,
        description="Best candidates selected for next iteration",
    )
    ...
    threshold: float = Field(
        default=0.9,
        description="Score threshold for early termination",
    )

    # Control flow
    should_terminate: bool = Field(
        default=False,
        description="Whether to terminate search",
    )
    termination_reason: str | None = Field(
        default=None,
        description="Why search was terminated",
    )
    best_solution: ScoredCandidate | None = Field(
        default=None,
        description="Best solution found so far",
    )
    ...
    current_candidate_id: str | None = Field(
        default=None,
        description="ID of candidate being scored",
    )
    ...
    def convert_candidates(cls, data: dict[str, Any]) -> dict[str, Any]:
        """Convert raw candidate data to proper Candidate/ScoredCandidate.
...
class ExpansionState(ToTState):
    ...
    seed: ScoredCandidate | None = Field(
        default=None,
        description="Parent candidate to expand from",
    )
