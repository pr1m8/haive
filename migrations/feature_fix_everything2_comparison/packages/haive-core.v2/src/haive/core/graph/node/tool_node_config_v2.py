...
class ToolNodeConfig(BaseNodeConfig[TInput, TOutput]):
    ...
    handle_tool_errors: bool | str | Callable[..., str] | tuple[
        type[Exception], ...] = Field(
            default=True,
    ...
    def validate_tool_source(self) -> Self:
        ...
        if not self.tools and not self.engine_name:
            raise ValueError(
                "Either 'tools' or 'engine_name' must be provided")
        return self
    ...
    def __call__(self,
        ...
        logger.info(
            f"Found {len(last_message.tool_calls)} tool calls to process")
        ...
        # Execute tools
        try:
            result = self._execute_tools(state, filtered_tools, messages,
                                         config)
            return result
    ...
    def _get_tool_routes(self, state: StateLike) -> dict[str, str]:
        ...
        # Then try engine
        engine = self._get_engine_from_state(
            state) if self.engine_name else None
    ...
    def _filter_tools_by_route(
        ...
        for tool in tools:
            ...
            if route in self.allowed_routes:
                ...
                logger.debug(
                    f"Excluding tool '{tool_name}' (route: {route} not in {
                        self.allowed_routes})", )
    ...
    def _execute_tools(
        ...
        if isinstance(result, dict) and self.messages_field in result:
            ...
            tool_msg_count = sum(1 for msg in updated_messages[len(messages):]
            ...
            return Command(
                update={
                    self.messages_field: updated_messages,
    ...
    def _create_no_op_response(self) -> Command:
        ...
        return Command(update={self.error_field: None},
                       goto=self._get_goto_node())
        ...
        if self.create_error_messages and messages:
            ...
            if self._has_tool_calls(last_message):
                ...
                for tool_call in last_message.tool_calls:
                    ...
                    tool_id = (tool_call.get('id', f"call_{tool_name}") if
                               isinstance(tool_call, dict) else tool_call.id)
...
def create_langchain_tool_node(
    ...
    **kwargs,
    ...
    return LangChainToolNode(name=name,
                             engine_name=engine_name,
                             tools=tools,
                             **kwargs)
...
def create_function_tool_node(
    ...
    **kwargs,
    ...
    return FunctionToolNode(name=name,
                            engine_name=engine_name,
                            tools=tools,
                            **kwargs)
...
def create_pydantic_tool_node(
    ...
    **kwargs,
    ...
    return PydanticToolNode(name=name,
                            engine_name=engine_name,
                            tools=tools,
                            **kwargs)
