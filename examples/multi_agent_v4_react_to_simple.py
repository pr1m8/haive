...
async def main():
    ...
    config = AugLLMConfig(
        temperature=0.3,
        max_tokens=1000,  # Low temperature for consistent results
    )

    # Step 1: Create ReactAgent for reasoning and analysis
    analyzer = ReactAgent(
        name='analyzer', engine=config, tools=[
            calculator, word_counter, sentiment_analyzer], system_message=(
    ...
    formatter = SimpleAgent(
        name='formatter', engine=config, structured_output_model=AnalysisReport, system_message=(
...
async def advanced_example():
    ...
    test_cases = [{'messages': [{'role': 'user',
                                 'content': 'What is 2 + 2? This is a simple question.'},
                                ],
                   },
                  {'messages': [{'role': 'user',
                   'content': ('This is a complex analysis task. Calculate the compound interest on $10,000 '
