...
try:
    ...
    logger.error("langchain_core not found. Please install it with: pip install langchain-core")
    sys.exit(1)
...
def load_tools_from_module(module_path: str, tool_type: str) -> List[BaseTool]:
    ...
    try:
        ...
        if module_env_vars:
            logger.info(f"Module {module_path} may require environment variables: {', '.join(module_env_vars)}")
        ...
        for name, obj in inspect.getmembers(module):
            ...
            if isinstance(obj, list) and all(isinstance(item, BaseTool) for item in obj):
                logger.info(f"Found tool list '{name}' in {module_path} with {len(obj)} tools")
                for tool in obj:
                    # Set up metadata
                    ...
                    try:
                        ...
                        logger.warning(f"Error extracting schema for tool {tool.name}: {e}")
...
def get_or_create_category(name: str, display_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        response = table(supabase, "tools.categories").select("id").eq("name", name).execute()
...
def get_or_create_toolkit(name: str, display_name: Optional[str] = None, description: Optional[str] = None) -> str:
    ...
    try:
        ...
        response = table(supabase, "tools.toolkits").select("id").eq("name", name).execute()
        ...
        if response.data and len(response.data) > 0:
            ...
            table(supabase, "tools.toolkits").update(update_data).eq("id", toolkit_id).execute()
...
def get_or_create_tool(
    ...
    try:
        ...
        response = table(supabase, "tools.tools").select("id").eq("name", name).execute()
        ...
        if response.data and len(response.data) > 0:
            ...
            table(supabase, "tools.tools").update(update_data).eq("id", tool_id).execute()
...
def register_environment_variables(tool_id: str, env_vars: List[str]) -> None:
    ...
    try:
        ...
        for env_name in env_vars:
            ...
            response = table(supabase, "config.environment_variables").select("id").eq("name", env_name).execute()
            ...
            if response.data and len(response.data) > 0:
                ...
                response = table(supabase, "config.environment_variables").insert(env_data).execute()
                
                if response.data and len(response.data) > 0:
                    env_var_ids.append(response.data[0]["id"])
        
        # Get tool as component
        response = table(supabase, "config.components").select("id").eq("component_id", tool_id).eq("component_type", "TOOL").execute()
        ...
        if response.data and len(response.data) > 0:
            ...
            response = table(supabase, "config.components").insert(component_data).execute()
        ...
        if component_id:
            for env_var_id in env_var_ids:
                ...
                response = table(supabase, "config.component_env_mappings").select("id").eq("component_id", component_id).eq("env_var_id", env_var_id).execute()
                ...
                table(supabase, "config.component_env_mappings").insert(mapping_data).execute()
    
    except Exception as e:
        logger.error(f"Error registering environment variables for tool {tool_id}: {e}")

def link_tool_to_toolkit(tool_id: str, toolkit_id: str) -> bool:
    """Link a tool to a toolkit."""
    try:
        # Check if link already exists
        response = table(supabase, "tools.toolkit_tools").select("id").eq("toolkit_id", toolkit_id).eq("tool_id", tool_id).execute()
...
def import_tools_to_database():
    ...
    for tool in all_tools:
        try:
            ...
            logger.error(f"Error importing tool {getattr(tool, 'name', 'unknown')}: {e}")
    ...
    logger.info(f"Imported {imported_tools} tools into {len(imported_toolkits)} toolkits")
...
def print_tool_stats():
    ...
    sorted_modules = sorted(tools_by_module.items(), key=lambda x: len(x[1]), reverse=True)
    for module, tools in sorted_modules[:10]:
        print(f"  • {module}: {len(tools)} tools")
    
    # Count tools with env vars
    tools_with_env_vars = sum(1 for tool in all_tools if 
                             tool.metadata.get("module_env_vars") or 
                             tool.metadata.get("tool_env_vars"))
    print(f"\n🔑 {tools_with_env_vars} tools require environment variables")
    
    # Count tools with schemas
    tools_with_schemas = sum(1 for tool in all_tools if tool.metadata.get("args_schema_json"))
    print(f"📋 {tools_with_schemas} tools have input schemas")
...
def main():
    ...
    tools_from_toolkits = load_tools_from_directory(TOOLKITS_PATH, "haive.tak.toolkits", "toolkit")
