...
class SelfDiscoverAgentConfig(AgentArchitectureConfig):
    ...
    state_schema: SelfDiscoverState = Field(default=SelfDiscoverState,description="State schema")
...
class SelfDiscoverAgent(AgentArchitecture):
    ...
    def adapt(self, inputs):
        ...
        if isinstance(adapted_modules, str):
            adapted_modules = AdaptedModules(
                adapted_modules=[
                    AdaptedModule(adapted_module=mod.name) for mod in inputs["selected_modules"].modules
                ]
            )
    ...
    def structure(self, inputs):
        ...
        return Command(update={"reasoning_structure": reasoning_plan, "plan": reasoning_plan})
    ...
    async def reason(self,state: SelfDiscoverState):
        ...
        reasoning_input = {
            ...
            "reasoning_modules": "\n".join([f"- {mod.name}: {mod.description}" for mod in next_step.reasoning_modules])
        }

        # Invoke the reasoning model
        reasoning_response = await self.aug_llm_model_runnables_dict["step_reasoning"].ainvoke(reasoning_input)
    ...
    async def run(self, task_description: str):
        ...
        """
        async for s in self.app.astream({"task_description": task_description}, config=self.runnable_config):
            print(s)
