...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"haive_vault_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
def run_migrate():
    ...
    try:
        from src.haive.dataflow.registry.utils.vault_migration_script import migrate_env_vars_to_vault, migrate_component_env_mappings, \
                                         migrate_provider_api_keys, migrate_engine_api_keys, add_vault_helper_functions
        ...
        logger.error("Could not import vault migration script. Make sure it's in the current directory.")
        return 1
...
def run_import(model_type=None, skip_llm=False, skip_embeddings=False, no_progress=False):
    """Run the model importer."""
    try:
        from src.haive.dataflow.registry.importers.litellm_importer import import_llm_models, import_embedding_models, update_availability_status
        ...
        if not skip_embeddings and (model_type is None or model_type.lower() == 'embedding'):
        ...
        logger.error("Could not import unified importer script. Make sure it's in the current directory.")
        return 1
    except Exception as e:
...
def run_verify():
    ...
    try:
        from src.haive.dataflow.registry.utils.vault_migration_script import generate_report
        ...
        logger.error("Could not import verification script. Make sure it's in the current directory.")
        return 1
    except Exception as e:
...
def main():
    ...
    # Migrate command
    migrate_parser = subparsers.add_parser("migrate", help="Migrate API keys and secrets to the vault")
    
    # Import command
    import_parser = subparsers.add_parser("import", help="Import LLM and embedding models")
    import_parser.add_argument("--model-type", choices=["llm", "embedding"], help="Type of models to import (default: both)")
    import_parser.add_argument("--skip-llm", action="store_true", help="Skip LLM model import")
    import_parser.add_argument("--skip-embeddings", action="store_true", help="Skip embedding model import")
    import_parser.add_argument("--no-progress", action="store_true", help="Disable progress bars")
    
    # Verify command
    verify_parser = subparsers.add_parser("verify", help="Verify vault secret references")
    ...
    if args.command == "migrate":
        ...
        return run_import(args.model_type, args.skip_llm, args.skip_embeddings, args.no_progress)
