...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"vault_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
def execute_sql(sql: str) -> Any:
    ...
    try:
        ...
        try:
            from src.haive.dataflow.db.supabase import sanitize_sql, get_supabase_client
    ...
    if module_path:
        ...
        if migration_module and hasattr(migration_module, "main"):
            try:
                ...
                migration_module.ensure_vault_reference_column = ensure_vault_reference_column
...
def run_verify(args):
    ...
    if module_path:
        ...
        if verify_module and hasattr(verify_module, "generate_report"):
            ...
            logger.error("Could not find generate_report function in verification module")
            return 1
    else:
...
def add_columns(args):
    ...
    try:
        ...
        if not hasattr(result, 'error') or not result.error:
            ...
            logger.error(f"Failed to create get_vault_secret_by_name function: {result.error}")
            success = False
    except Exception as e:
...
def run_export(args):
    ...
    if not module_path:
        ...
        try:
            ...
            # Export to file
            export_file = os.path.join(export_dir, f"vault_secrets_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        ...
        if export_module and hasattr(export_module, "export_secrets"):
            try:
                export_module.export_secrets(output_dir=args.output_dir, env_file=args.env_file)
                return 0
            except Exception as e:
...
def run_import_secrets(args):
    ...
    if not module_path:
        ...
        try:
            ...
            for secret_data in import_data:
                try:
                    ...
                    description = secret_data.get("description", f"Imported secret: {name}")
                    ...
                    response = table(supabase, "vault.secrets").select("id").eq("name", name).execute()
                    ...
                    if response.data and len(response.data) > 0:
                        ...
                        if args.overwrite:
                            ...
                            # Update the secret
                            update_response = table(supabase, "vault.secrets").update({
                        ...
                        # Create new secret
                        insert_response = table(supabase, "vault.secrets").insert({
                            "name": name,
                            "description": description,
                            "secret": value,
            ...
            logger.info(f"Import completed: {imported_count} secrets imported, {updated_count} secrets updated")
        ...
        if import_module and hasattr(import_module, "import_secrets"):
            try:
                import_module.import_secrets(input_file=args.input_file, overwrite=args.overwrite)
                return 0
            except Exception as e:
...
def main():
    ...
    # Migrate command
    migrate_parser = subparsers.add_parser("migrate", help="Migrate API keys and secrets to vault")
    migrate_parser.set_defaults(func=run_migrate)
    
    # Import command
    import_parser = subparsers.add_parser("import-models", help="Import LLM and embedding models")
    import_parser.add_argument("--model-type", choices=["llm", "embedding", "all"], default="all", help="Type of models to import")
    import_parser.add_argument("--skip-llm", action="store_true", help="Skip LLM models")
    import_parser.add_argument("--skip-embeddings", action="store_true", help="Skip embedding models")
    ...
    # Add columns command
    columns_parser = subparsers.add_parser("add-columns", help="Add vault reference columns to database tables")
    ...
    export_parser.add_argument("--env-file", action="store_true", help="Create .env file from secrets")
    export_parser.set_defaults(func=run_export)
    
    # Import secrets command
    import_secrets_parser = subparsers.add_parser("import-secrets", help="Import secrets into vault")
    import_secrets_parser.add_argument("--input-file", required=True, help="JSON file containing secrets to import")
    import_secrets_parser.add_argument("--overwrite", action="store_true", help="Overwrite existing secrets")
