...
def ensure_registry_schema(client):
    ...
    try:
        ...
        if not result.data or len(result.data) == 0:
            ...
            client.rpc(
                "execute_sql",
                {"sql": "CREATE SCHEMA IF NOT EXISTS models"}
            ).execute()
...
def ensure_provider_types(client):
    ...
    try:
        ...
        provider_types = client.rpc("execute_sql", {"sql": provider_types_query}).execute()
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"  ✅ Found {len(EMBEDDING_MODELS)} embedding models in embeddings_importer")
            
            # Show a sample of embedding models
            sample_size = min(3, len(EMBEDDING_MODELS))
            logger.info(f"  Sample of embedding models:")
            for i in range(sample_size):
                model = EMBEDDING_MODELS[i]
                logger.info(f"    - {model['model_id']} ({model['dimensions']} dimensions, {model['provider']} provider)")
        except ImportError:
        ...
        try:
            ...
            logger.info(f"  ✅ LiteLLM importer is available (imports models from GitHub)")
        except ImportError:
        ...
        logger.info(f"  Available LLM providers: {[p.get('name') for p in llm_providers]}")
        
        # Get available embedding providers
        embed_providers = client.get_available_embedding_providers()
        logger.info(f"  Available embedding providers: {[p.get('name') for p in embed_providers]}")
        ...
        logger.info(f"  Found {len(available_embed_models)} available embedding models (out of {len(all_embed_models)} total)")
        ...
        logger.info(f"  Found {len(available_llm_models)} available LLM models (out of {len(all_llm_models)} total)")
        ...
        if available_embed_models:
            ...
            logger.info(f"  Max tokens: {sample_embed.get('max_input_tokens', 'Unknown')}")
            logger.info(f"  Pricing: {sample_embed.get('pricing', {}).get('input_cost_per_token', 'Unknown')} per token")
