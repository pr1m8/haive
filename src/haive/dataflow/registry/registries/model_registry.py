...
try:
    from src.haive.core.models.llm.base import LLMConfig, SecureConfigMixin
...
class ModelRegistry:
    ...
    def _store_secret_in_vault(self, env_var_id: str, var_name: str, provider_id: str | None = None):
    ...
    def _get_default_team_id(self, user_id: str) -> str | None:
        ...
        if self._supabase:
            try:
                ...
                try:
                    ...
                    logger.exception(f"Error retrieving providers with table method: {table_e}")
    ...
    def get_available_embedding_providers(self) -> List[Dict[str, Any]]:
        ...
        if self._supabase:
            try:
                ...
                logger.exception(f"Error retrieving available embedding providers using RPC: {e}")
