...
REWOO_SYSTEM_PROMPT = """You are a planning agent that creates detailed, structured plans.
...
REWOO_USER_PROMPT = """Create a detailed plan for the following task. Your response must be valid JSON matching this structure:
{{
...
REWOO_OUTPUT_PARSER = PydanticOutputParser(pydantic_object=RewooPlan)

REWOO_PROMPT_TEMPLATE = ChatPromptTemplate.from_messages(
            [
                ("system", REWOO_SYSTEM_PROMPT),
                ("user", REWOO_USER_PROMPT)
            ]
        ).partial(
            format_instructions=REWOO_OUTPUT_PARSER.get_format_instructions()
        )
rewoo_aug_llm_config = AugLLMConfig(
    name="rewoo_agent",
    prompt_template=REWOO_PROMPT_TEMPLATE,
...
solve_aug_llm_config = AugLLMConfig(
    name="rewoo_solve",
    prompt_template=REWOO_SOLVE_PROMPT_TEMPLATE,
    output_parser=PydanticOutputParser(pydantic_object=RewooPlan)
)