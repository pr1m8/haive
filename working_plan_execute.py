"""Working Plan & Execute Implementation with Engine field fix.

This implementation uses a fixed state schema that avoids the Engine serialization
...
from haive.agents.planning.p_and_e.models import Plan, Act, StepStatus
...
def should_continue(state: FixedPlanExecuteState) -> str:
...
def route_after_replan(state: FixedPlanExecuteState) -> str:
...
def process_planner_output(state: FixedPlanExecuteState, planner_result) -> dict:
...
def process_executor_output(state: FixedPlanExecuteState, executor_result) -> dict:
...
def process_replanner_output(state: FixedPlanExecuteState, replanner_result) -> dict:
...
def create_working_plan_execute(
    name: str = "WorkingPlanExecute",
    planner_model: str = "gpt-4o-mini",
    executor_model: str = "gpt-4o-mini",
    replanner_model: str = "gpt-4o-mini",
    tools: List = None
) -> MultiAgentBase:
    """Create working Plan & Execute agent with Engine field fix.
    ...
    Returns:
        MultiAgentBase: Complete Plan & Execute system that actually works
    """
    if tools is None:
        tools = []
    ...
    # Create MultiAgentBase with fixed state schema
    return MultiAgentBase(
        agents=[planner, executor, replanner],
        branches=branches,
        entry_points=[planner],  # Start with planning
        name=name,
        state_schema_override=FixedPlanExecuteState,  # Use fixed state
...
def create_working_plan_execute_with_search(tools: List = None) -> MultiAgentBase:
    """Create working Plan & Execute agent with search tools."""
    if tools is None:
        from haive.tools import duckduckgo_search_tool
        tools = [duckduckgo_search_tool]
    
    return create_working_plan_execute(
        name="WorkingPlanExecuteWithSearch",
        tools=tools
    )
...
if __name__ == "__main__":
    print("🚀 Testing Working Plan & Execute Agent")
    print("=" * 50)
    ...
    print("\n" + "=" * 50)
    print("🎯 Key Fix: Removed Engine fields from state schema")
    print("✅ This prevents the 'Can't instantiate abstract class Engine' error")
    print("✅ Plan & Execute agent now works correctly!")