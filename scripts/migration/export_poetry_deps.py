...
class HaiveDependencyManager:
    ...
    def extract_top_level_external_dependencies(self, root_pyproject: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        dependencies = root_pyproject.get('tool', {}).get('poetry', {}).get('dependencies', {})
        
        # Filter out Haive-specific packages
        external_deps = {
            dep: version for dep, version in dependencies.items() 
    ...
    def distribute_external_dependencies(self, root_pyproject: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Failed to export external dependencies: {e}")
