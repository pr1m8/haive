...
def generate_setup_py(pyproject_path):
    ...
    author_email = authors[0].split('<')[1].split('>')[0].strip() if authors and '<' in authors[0] else ''
    ...
    for dep_name, dep_version in package_info.get('dependencies', {}).items():
        if dep_name != 'python' and not isinstance(dep_version, dict):
            ...
            if isinstance(dep_version, str) and dep_version.startswith('^'):
                major, minor, patch = re.match(r'\^(\d+)\.(\d+)\.(\d+)', dep_version).groups()
                dependencies[dep_name] = f'>={major}.{minor}.{patch},<{int(major)+1}.0.0'
            else:
...
""")
    ...
    print(f"Generated setup.py for {name}")
    return True

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python generate_setup.py <pyproject.toml>")
        sys.exit(1)
