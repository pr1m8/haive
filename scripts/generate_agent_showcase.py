...
class AgentDiscovery:
    ...
    def analyze_class_node(self, node: ast.ClassDef, file_path: Path, module_docstring: str) -> Optional[AgentInfo]:
    ...
    def categorize_agent(self, file_path: Path, class_name: str, docstring: str) -> str:
        ...
        if 'haive-games' in path_str:
            ...
            elif any(game in path_str for game in ['among_us', 'mafia', 'risk']):
                return 'Strategy Games'
            elif any(game in path_str for game in ['cards', 'blackjack']):
                return 'Card Games'
            else:
                return 'Games'
                
        elif 'haive-prebuilt' in path_str:
            if any(term in path_str for term in ['contract', 'legal']):
                return 'Legal & Business'
            elif any(term in path_str for term in ['scientific', 'research', 'paper']):
                return 'Academic & Research'
            else:
                return 'Prebuilt Solutions'
                
        elif 'haive-agents' in path_str:
    ...
    def determine_complexity(self, node: ast.ClassDef, docstring: str, features: List[str]) -> str:
        ...
        if method_count > 10 or has_complex_features or has_complex_keywords:
            return 'complex'
        elif method_count > 5 or len(features) > 3:
            return 'medium'
        else:
            return 'simple'
