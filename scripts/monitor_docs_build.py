...
class BuildDashboard:
    ...
    def load_monitor_data(self) -> Optional[Dict[str, Any]]:
        ...
        while True:
            ...
            for name, stage in stage_details.items():
                ...
                if stage["status"] in ["success", "failed"]:
                    ...
                    stdscr.addstr(
                        y, 50, f"{stage['progress_percent']:.1f}%", curses.color_pair(3)
                    )
...
def find_latest_monitor_file(log_dir: Path) -> Optional[Path]:
