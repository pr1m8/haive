...
class GithubSettings(BaseSettings):
    ...
    github_token: SecretStr | None = Field(default=None, description='GitHub personal access token from environment', alias='GITHUB_TOKEN')
    github_api_token: SecretStr | None = Field(default=None, description='Alternative GitHub API token', alias='GITHUB_API_TOKEN')
    github_pat: SecretStr | None = Field(default=None, description='GitHub Personal Access Token', alias='GITHUB_PAT')
    github_default_branch: str = Field(default='main', description='Default branch name to try first', alias='GITHUB_DEFAULT_BRANCH')
    github_fallback_branches: list[str] = Field(default=['main', 'master', 'develop', 'development', 'prod', 'production'], description='Branches to try if default fails', alias='GITHUB_FALLBACK_BRANCHES')
    github_api_timeout: int = Field(default=10, description='Timeout for GitHub API calls in seconds', alias='GITHUB_API_TIMEOUT')
    github_auto_retry_branches: bool = Field(default=True, description="Automatically try fallback branches if specified branch doesn't exist", alias='GITHUB_AUTO_RETRY_BRANCHES')
...
class GithubRepo(BaseModel):
    ...
    """
    owner: str | None = Field(default=None, description='GitHub username or organization that owns the repository', min_length=1, max_length=39, examples=['microsoft', 'facebook', 'openai'])
    name: str | None = Field(default=None, description='Repository name', min_length=1, max_length=100, examples=['vscode', 'react', 'langchain'])
    url: HttpUrl | None = Field(default=None, description='Any GitHub URL (repo, file, API, etc.) - will be parsed to extract owner/name', examples=['https://github.com/microsoft/vscode', 'https://github.com/langchain-ai/langchain/blob/main/README.md', 'https://api.github.com/repos/openai/gpt-3'])
    is_private: bool | None = Field(default=None, description='Whether the repository is private (auto-detected if not specified)')
    branch: str | None = Field(default=None, description='Preferred branch (will fallback to main/master if not found)', min_length=1)
    access_token: SecretStr | None = Field(default=None, description='GitHub personal access token (falls back to environment variables)')
    api_url: HttpUrl | None = Field(default=None, description='GitHub API URL (auto-generated)', exclude=True)
    clone_url: HttpUrl | None = Field(default=None, description='Git clone URL (auto-generated)', exclude=True)
    default_branch: str | None = Field(default=None, description='The actual default branch from GitHub (auto-discovered)')
    discovered_branch: str | None = Field(default=None, description='The branch that was actually found and validated')
    available_branches: list[str] = Field(default_factory=list, description='List of available branches discovered during validation')
    is_valid: bool = Field(default=False, description='Whether the repository has been validated to exist')
    validation_error: str | None = Field(default=None, description='Error message if validation failed')
    validation_warnings: list[str] = Field(default_factory=list, description='Warnings from validation (e.g., branch fallback used)')
    ...
    auto_retry_branches: bool = Field(default=True, description="Automatically try fallback branches if specified branch doesn't exist")
    fallback_branches: list[str] = Field(default_factory=lambda: ['main', 'master'], description="Branches to try if the specified branch doesn't exist")
    ...
    def validate_github_identifier(cls, v: str | None) -> str | None:
        ...
        if not re.match('^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$', v):
            raise ValueError(f"Invalid GitHub identifier format: '{v}'. Must start with alphanumeric, can contain hyphens, and must end with alphanumeric.")
        return v
    ...
    def extract_from_url_and_validate(self) -> Self:
        ...
        if not self.owner or not self.name:
            raise ValueError("Either provide 'owner' and 'name', or a valid GitHub 'url' to parse")
        ...
        try:
            ...
            logger.exception(f'Repository validation failed for {self.owner}/{self.name}: {e}')
        return self
    ...
    def _parse_github_url(self, url: str) -> tuple[str | None, str | None]:
        ...
        patterns = ['github\\.com/([^/]+)/([^/\\s]+?)(?:\\.git)?/?$', 'github\\.com/([^/]+)/([^/]+)/(?:blob|tree|commit|pulls|issues)', 'api\\.github\\.com/repos/([^/]+)/([^/\\s]+)', 'gist\\.github\\.com/([^/]+)/([a-f0-9]+)']
    ...
    def _validate_and_discover(self) -> None:
        ...
        try:
            response = self.http_client.get(str(self.api_url), headers=headers, follow_redirects=True)
            if response.status_code == 404:
                raise ValueError(f'Repository {self.owner}/{self.name} not found')
            if response.status_code == 403:
                if not self.access_token:
                    raise ValueError('GitHub API rate limit exceeded or repository is private. Please provide an access token.')
            ...
            if data.get('updated_at'):
                self.last_updated = datetime.fromisoformat(data['updated_at'].replace('Z', '+00:00'))
    ...
    def _discover_and_validate_branch(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f'Found {len(self.available_branches)} branches for {self.full_name}')
        except Exception as e:
            ...
            for branch in branches_to_try:
                if self._check_branch_exists(branch):
                    ...
                    if branch != self.branch and self.branch:
                        self.validation_warnings.append(f"Specified branch '{self.branch}' not found, using '{branch}' instead")
                    break
            if valid_branch:
                ...
                if self.available_branches:
                    available_msg = f' Available branches: {', '.join(self.available_branches[:5])}'
                    if len(self.available_branches) > 5:
                        available_msg += f' and {len(self.available_branches) - 5} more'
                raise ValueError(f'No valid branch found. Tried: {tried_branches}.{available_msg}')
        elif self.branch:
            if not self._check_branch_exists(self.branch):
                raise ValueError(f"Branch '{self.branch}' not found and auto-retry is disabled")
    ...
    def _check_branch_exists(self, branch: str) -> bool:
        ...
        headers = self.get_api_headers()
        try:
            response = self.http_client.get(f'{self.api_url}/branches/{branch}', headers=headers)
    ...
    def get_api_headers(self) -> dict[str, str]:
        ...
        headers = {'Accept': 'application/vnd.github.v3+json', 'User-Agent': f'Haive-Framework/{self.full_name}'}
    ...
    def to_safe_dict(self) -> dict[str, Any]:
        ...
        data = self.model_dump(exclude={'access_token', 'api_url', 'clone_url', '_settings', '_http_client', '_branch_cache'})
