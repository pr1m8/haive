...
class ToolAlias(BaseModel):
    ...
    actual_tool: str = Field(..., description='The actual tool name to execute', min_length=1, max_length=50)
    force_choice: bool = Field(default=True, description='Whether to force this tool choice')
    parameters: dict[str, Any] = Field(default_factory=dict, description='Default parameters for the tool')
...
class PlanNode(BaseModel):
    ...
    name: str = Field(..., description='Human-readable name for the task', min_length=1, max_length=200)
    task_type: TaskType = Field(default=TaskType.EXECUTION, description='Type of task this node represents')
    description: str = Field(..., description='Detailed description of the task', min_length=1, max_length=1000)
    status: TaskStatus = Field(default=TaskStatus.PENDING, description='Current status of the task')
    priority: TaskPriority = Field(default=TaskPriority.MEDIUM, description='Priority level of the task')
    agent_name: str | None = Field(default=None, description='Name of the agent to execute this task')
    ...
    dependencies: list[str] = Field(default_factory=list, description='List of node IDs this task depends on')
    dependent_nodes: list[str] = Field(default_factory=list, description='List of node IDs that depend on this task')
    ...
    parallelizable: bool = Field(default=True, description='Whether this task can be executed in parallel')
    estimated_duration: float | None = Field(default=None, description='Estimated execution time in seconds')
...
class PlanTree(BaseModel):
    ...
    model_config = ConfigDict(str_strip_whitespace=True, validate_assignment=True, extra='forbid')
    id: str = Field(..., description='Unique identifier for the plan tree', min_length=1, max_length=100)
    name: str = Field(..., description='Human-readable name for the plan', min_length=1, max_length=200)
    description: str = Field(..., description='Description of what this plan accomplishes', min_length=1, max_length=1000)
    nodes: dict[str, PlanNode] = Field(default_factory=dict, description='Dictionary of all nodes in the tree')
    ...
    execution_levels: list[list[str]] = Field(default_factory=list, description='Nodes organized by execution level for parallelization')
    max_parallelism: int = Field(default=4, description='Maximum number of parallel executions', ge=1, le=16)
    ...
    def get_ready_nodes(self) -> list[PlanNode]:
        ...
        completed_ids = {node_id for node_id, node in self.nodes.items() if node.status == TaskStatus.COMPLETED}
        ready_nodes = []
    ...
    def mark_node_completed(self, node_id: str, result: Any=None) -> None:
        ...
        if node_id in self.nodes:
            ...
            self.completed_nodes = sum((1 for node in self.nodes.values() if node.status == TaskStatus.COMPLETED))

    def mark_node_failed(self, node_id: str, error: str) -> None:
        """Mark a node as failed."""
        if node_id in self.nodes:
            self.nodes[node_id].mark_failed(error)
            self.failed_nodes = sum((1 for node in self.nodes.values() if node.status == TaskStatus.FAILED))
...
class ReWOOTreePlannerOutput(BaseModel):
    ...
    model_config = ConfigDict(str_strip_whitespace=True, validate_assignment=True, extra='forbid')
    plan_id: str = Field(..., description='Unique identifier for the generated plan', min_length=1, max_length=100)
    plan_name: str = Field(..., description='Human-readable name for the plan', min_length=1, max_length=200)
    problem_analysis: str = Field(..., description='Analysis of the problem and requirements', min_length=10, max_length=2000)
    approach_strategy: str = Field(..., description='High-level strategy for solving the problem', min_length=10, max_length=1000)
    plan_tree: PlanTree = Field(..., description='The complete planning tree with all nodes')
    estimated_duration: float = Field(default=0.0, description='Estimated total execution time in seconds', ge=0.0)
    parallelization_factor: float = Field(default=1.0, description='Factor by which parallelization improves performance', ge=1.0, le=10.0)
    required_tools: list[str] = Field(default_factory=list, description='List of tools required for execution')
    tool_aliases: dict[str, ToolAlias] = Field(default_factory=dict, description='Tool aliases for forced tool choice')
    risk_factors: list[str] = Field(default_factory=list, description='Identified risk factors in the plan')
    fallback_strategies: list[str] = Field(default_factory=list, description='Fallback strategies if parts of the plan fail')
...
class ReWOOTreeExecutorOutput(BaseModel):
    ...
    model_config = ConfigDict(str_strip_whitespace=True, validate_assignment=True, extra='forbid')
    execution_id: str = Field(..., description='Unique identifier for this execution', min_length=1, max_length=100)
    plan_id: str = Field(..., description='ID of the plan being executed', min_length=1, max_length=100)
    completed_nodes: dict[str, Any] = Field(default_factory=dict, description='Results from completed nodes')
    failed_nodes: dict[str, str] = Field(default_factory=dict, description='Error messages from failed nodes')
    final_result: Any | None = Field(default=None, description='Final aggregated result of the execution')
    total_execution_time: float = Field(default=0.0, description='Total execution time in seconds', ge=0.0)
    parallel_efficiency: float = Field(default=0.0, description='Efficiency of parallel execution (0-1)', ge=0.0, le=1.0)
    ...
    completion_percentage: float = Field(default=0.0, description='Percentage of plan completed', ge=0.0, le=100.0)
...
class ReWOOTreeAgent(Agent):
    ...
    """
    available_tools: list[BaseTool] = Field(default_factory=list, description='Available tools for execution')
    tool_aliases: dict[str, ToolAlias] = Field(default_factory=dict, description='Tool aliases for forced choice')
    max_planning_depth: int = Field(default=5, description='Maximum depth for recursive planning', ge=1, le=10)
    ...
    executor_agent: Agent | None = Field(default=None, description='Specialized agent for execution')
    ...
    def _create_planner_agent(self) -> Agent:
        ...
        return SimpleAgent(name=f'{self.name}_planner', engine=AugLLMConfig(prompt_template=planner_prompt, temperature=0.3, structured_output_model=ReWOOTreePlannerOutput))
    ...
    def _create_executor_agent(self) -> Agent:
        ...
        return ReactAgent(name=f'{self.name}_executor', engine=AugLLMConfig(prompt_template=executor_prompt, temperature=0.5, structured_output_model=ReWOOTreeExecutorOutput), tools=self.available_tools)

    def add_tool_alias(self, alias: str, actual_tool: str, force_choice: bool=True, **params) -> None:
        """Add a tool alias for forced tool choice."""
        tool_alias = ToolAlias(alias=alias, actual_tool=actual_tool, force_choice=force_choice, parameters=params)
    ...
    def _planning_node(self, state: ReWOOTreeAgentState) -> Command:
        ...
        context = {'planning_depth': planning_depth, 'available_tools': [tool.name for tool in self.available_tools], 'tool_aliases': self.tool_aliases, 'max_parallelism': self.max_parallelism}
        user_input = ''
        if state.messages:
            user_input = state.messages[-1].content if hasattr(state.messages[-1], 'content') else str(state.messages[-1])
        planner_input = {'input': user_input, 'context': context, 'tools': [tool.name for tool in self.available_tools]}
        try:
            ...
            return Command(update={'current_plan': plan_result, 'tool_aliases': plan_result.tool_aliases, 'messages': [*state.messages, AIMessage(content=f'Created plan: {plan_result.plan_name}')]}, goto='executor')
        except Exception as e:
            logger.exception(f'Planning failed: {e}')
            fallback_plan = self._create_fallback_plan(str(e), user_input)
            return Command(update={'current_plan': fallback_plan, 'messages': [*state.messages, AIMessage(content=f'Created fallback plan due to error: {e}')]}, goto='executor')

    def _create_fallback_plan(self, error_or_result: str, user_input: str) -> ReWOOTreePlannerOutput:
        """Create a fallback plan when planning fails."""
        plan_id = f'fallback_{uuid.uuid4().hex[:8]}'
        root_node = PlanNode(id='root_task', name='Execute Request', task_type=TaskType.EXECUTION, description=f'Execute the user request: {user_input}', priority=TaskPriority.HIGH, parallelizable=False)
        plan_tree = PlanTree(id=plan_id, name='Fallback Plan', description='Simple fallback plan for execution', root_id='root_task')
        plan_tree.add_node(root_node)
        return ReWOOTreePlannerOutput(plan_id=plan_id, plan_name='Fallback Plan', problem_analysis=f'Analysis: {error_or_result}', approach_strategy='Execute as single task', plan_tree=plan_tree, estimated_duration=30.0, parallelization_factor=1.0, required_tools=[tool.name for tool in self.available_tools], tool_aliases=self.tool_aliases, risk_factors=['Simple fallback execution'], fallback_strategies=['Direct execution without complex planning'])
    ...
    def _execution_coordinator_node(self, state: ReWOOTreeAgentState) -> Command:
        ...
        if not state.current_plan:
            return Command(update={'messages': [*state.messages, AIMessage(content='No plan to execute')]}, goto='aggregator')
        ...
        execution_output = ReWOOTreeExecutorOutput(execution_id=f'exec_{plan_tree.id}', plan_id=plan_tree.id, completed_nodes=execution_results, failed_nodes={node_id: node.error for node_id, node in plan_tree.nodes.items() if node.status == TaskStatus.FAILED}, nodes_executed=len(execution_results), nodes_failed=plan_tree.failed_nodes, success=plan_tree.completed_nodes > 0 and plan_tree.failed_nodes == 0, completion_percentage=plan_tree.get_completion_percentage())
        return Command(update={'current_execution': execution_output, 'node_results': execution_results, 'messages': [*state.messages, AIMessage(content=f'Execution completed: {execution_output.completion_percentage:.1f}% success')]}, goto='recursive_check')

    def _execute_nodes_sync(self, nodes: list[PlanNode], state: ReWOOTreeAgentState) -> dict[str, Any]:
    ...
    async def _execute_parallel_nodes(self, nodes: list[PlanNode], state: ReWOOTreeAgentState) -> dict[str, Any]:
    ...
    async def _execute_single_node(self, node: PlanNode, state: ReWOOTreeAgentState) -> Any:
        ...
        logger.info(f'🔧 Executing node: {node.name}')
        node.mark_started()
        execution_context = {'task': node.description, 'task_type': node.task_type, 'expected_output': node.expected_output, 'tool_alias': node.tool_alias, 'dependencies': [state.node_results.get(dep_id) for dep_id in node.dependencies if dep_id in state.node_results]}
        try:
    ...
    async def _execute_with_tool_alias(self, alias_config: ToolAlias, context: dict[str, Any]) -> Any:
    ...
    def _recursive_planning_check_node(self, state: ReWOOTreeAgentState) -> Command:
        ...
        if planning_depth < self.max_planning_depth and state.current_execution and (state.current_execution.completion_percentage < 80.0):
            failed_nodes = []
            if state.current_plan:
                failed_nodes = [node for node in state.current_plan.plan_tree.nodes.values() if node.status in [TaskStatus.FAILED, TaskStatus.BLOCKED]]
            if failed_nodes:
                logger.info(f'🔄 Triggering recursive planning for {len(failed_nodes)} failed nodes')
                return Command(update={'planning_depth': planning_depth + 1, 'subplan_stack': [*subplan_stack, state.current_plan.plan_id], 'messages': [*state.messages, AIMessage(content=f'Initiating recursive planning (depth {planning_depth + 1})')]}, goto='planner')
    ...
    def _result_aggregator_node(self, state: ReWOOTreeAgentState) -> Command:
        ...
        if state.current_execution and state.node_results:
            final_result = {'plan_summary': {'plan_id': state.current_plan.plan_id if state.current_plan else 'unknown', 'execution_id': state.current_execution.execution_id, 'success': state.current_execution.success, 'completion_percentage': state.current_execution.completion_percentage, 'nodes_executed': state.current_execution.nodes_executed, 'nodes_failed': state.current_execution.nodes_failed}, 'node_results': state.node_results, 'execution_levels': state.current_plan.plan_tree.execution_levels if state.current_plan else [], 'tool_usage': list(state.tool_aliases.keys()), 'planning_depth': state.planning_depth}
        response_content = self._format_final_response(final_result, state)
        return Command(update={'final_output': final_result, 'messages': [*state.messages, AIMessage(content=response_content)]}, goto='__end__')
