...
class QualitativeGrade(Grade):
    ...
    """
    grade_type: GradeType = Field(default=GradeType.QUALITATIVE, description='Type of grade model (always qualitative)')
    quality_level: QualityLevel = Field(..., description='Overall quality assessment level', examples=['good', 'excellent', 'fair'])
    sentiment: SentimentType = Field(default=SentimentType.NEUTRAL, description='Sentiment tone of the evaluation', examples=['positive', 'neutral', 'negative'])
    strengths: list[str] = Field(default_factory=list, description='List of identified strengths', max_length=10, examples=[['Clear writing', 'Good research', 'Strong conclusions'], ['Creative approach', 'Well-organized', 'Thorough analysis']])
    weaknesses: list[str] = Field(default_factory=list, description='List of identified weaknesses or areas for improvement', max_length=10, examples=[['Grammar issues', 'Weak introduction', 'Needs more examples'], ['Unclear thesis', 'Poor citations', 'Repetitive content']])
    recommendations: list[str] = Field(default_factory=list, description='List of specific recommendations for improvement', max_length=10, examples=[['Proofread carefully', 'Strengthen introduction', 'Add more examples'], ['Clarify main argument', 'Improve citation format', 'Reduce repetition']])
    detailed_feedback: str | None = Field(default=None, description='Extended qualitative feedback and commentary', max_length=5000, examples=['This essay demonstrates a strong understanding of the topic...', 'The analysis is thorough but could benefit from...'])
    ...
    def validate_feedback_consistency(self) -> 'QualitativeGrade':
        ...
        """
        quality_to_sentiment = {QualityLevel.EXCEPTIONAL: [SentimentType.VERY_POSITIVE, SentimentType.POSITIVE], QualityLevel.EXCELLENT: [SentimentType.VERY_POSITIVE, SentimentType.POSITIVE], QualityLevel.GOOD: [SentimentType.POSITIVE, SentimentType.NEUTRAL], QualityLevel.FAIR: [SentimentType.NEUTRAL, SentimentType.NEGATIVE], QualityLevel.POOR: [SentimentType.NEGATIVE, SentimentType.VERY_NEGATIVE], QualityLevel.UNACCEPTABLE: [SentimentType.NEGATIVE, SentimentType.VERY_NEGATIVE]}
        expected_sentiments = quality_to_sentiment[self.quality_level]
        if self.sentiment not in expected_sentiments:
            raise ValueError(f"Sentiment '{self.sentiment}' is inconsistent with quality level '{self.quality_level}'")
        return self
    ...
    def get_normalized_score(self) -> float:
        ...
        """
        quality_scores = {QualityLevel.EXCEPTIONAL: 0.95, QualityLevel.EXCELLENT: 0.85, QualityLevel.GOOD: 0.75, QualityLevel.FAIR: 0.65, QualityLevel.POOR: 0.45, QualityLevel.UNACCEPTABLE: 0.25}
    ...
    def is_passing(self, threshold: float | None=None) -> bool:
        ...
        if threshold is None:
            passing_levels = {QualityLevel.EXCEPTIONAL, QualityLevel.EXCELLENT, QualityLevel.GOOD, QualityLevel.FAIR}
    ...
    def get_feedback_summary(self) -> dict[str, Any]:
        ...
        """
        return {'quality_level': self.quality_level.value, 'sentiment': self.sentiment.value, 'strengths_count': len(self.strengths), 'weaknesses_count': len(self.weaknesses), 'recommendations_count': len(self.recommendations), 'has_detailed_feedback': bool(self.detailed_feedback), 'feedback_balance': self._calculate_feedback_balance(), 'improvement_areas': self.weaknesses[:3], 'key_strengths': self.strengths[:3]}
    ...
    def get_improvement_priority(self) -> list[str]:
        ...
        if len(self.recommendations) > len(self.weaknesses):
            ...
            improvements.extend(remaining_recs[:2])
    ...
    def generate_narrative_summary(self) -> str:
        ...
        """
        quality_desc = {QualityLevel.EXCEPTIONAL: 'exceptional work that exceeds expectations', QualityLevel.EXCELLENT: 'excellent work that meets all expectations', QualityLevel.GOOD: 'good work that meets most expectations', QualityLevel.FAIR: 'fair work that meets basic expectations', QualityLevel.POOR: 'work that falls below expectations', QualityLevel.UNACCEPTABLE: 'work that does not meet minimum standards'}
    ...
    def to_display_string(self) -> str:
        ...
        """
        quality_emoji = {QualityLevel.EXCEPTIONAL: '🌟', QualityLevel.EXCELLENT: '✨', QualityLevel.GOOD: '✅', QualityLevel.FAIR: '⚖️', QualityLevel.POOR: '⚠️', QualityLevel.UNACCEPTABLE: '❌'}
    ...
    @classmethod
    def create_positive_feedback(cls, justification: str, strengths: list[str], minor_improvements: list[str] | None=None, quality_level: QualityLevel=QualityLevel.GOOD, **kwargs) -> 'QualitativeGrade':
        ...
        """
        return cls(quality_level=quality_level, sentiment=SentimentType.POSITIVE, strengths=strengths, weaknesses=minor_improvements or [], justification=justification, **kwargs)

    @classmethod
    def create_constructive_feedback(cls, justification: str, strengths: list[str], weaknesses: list[str], recommendations: list[str], quality_level: QualityLevel=QualityLevel.FAIR, **kwargs) -> 'QualitativeGrade':
        ...
        """
        return cls(quality_level=quality_level, sentiment=SentimentType.NEUTRAL, strengths=strengths, weaknesses=weaknesses, recommendations=recommendations, justification=justification, **kwargs)