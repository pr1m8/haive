...
class Agent(TypedInvokableEngine[EngineT], ExecutionMixin, StateMixin, PersistenceMixin, SerializationMixin, StructuredOutputMixin, PrePostAgentMixin, ABC):
    ...
    """
    engine_type: Literal[EngineType.AGENT] = Field(default=EngineType.AGENT, description='Engine type, always AGENT')
    ...
    state_schema: type[BaseModel] | dict[str, Any] | None = Field(default=None, exclude=True, description='Schema for agent state')
    ...
    persistence: Any | None = Field(default=True, description='Persistence configuration for state checkpointing')
    ...
    def _auto_derive_io_schemas(self) -> None:
        ...
        if not self.output_schema:
            ...
            if main_engine:
                if hasattr(main_engine, 'structured_output_model') and main_engine.structured_output_model:
    ...
    def execute_hooks(self, event: HookEvent, **context_kwargs) -> list[Any]:
        ...
        if not self.hooks_enabled or event not in self._hooks or (not self._hooks[event]):
            return []
        context = HookContext(event=event, agent_name=self.name, agent_type=self.__class__.__name__, **context_kwargs)
        results = []
    ...
    def _trigger_auto_recompile(self) -> None:
        ...
        if self.hooks_enabled:
            self.execute_hooks(HookEvent.BEFORE_BUILD_GRAPH, metadata={'compilation_type': 'auto_recompile'})
        try:
            self.graph = self.build_graph()
            self._graph_built = True
            ...
            if self.hooks_enabled:
                self.execute_hooks(HookEvent.AFTER_BUILD_GRAPH, metadata={'compilation_type': 'auto_recompile', 'graph': self.graph})
        except Exception as e:
            logger.exception(f"Auto-recompile failed for agent '{self.name}': {e}")
            self.resolve_recompile(success=False)
            if self.hooks_enabled:
                self.execute_hooks(HookEvent.ON_ERROR, error=e, metadata={'compilation_type': 'auto_recompile'})
    ...
    def compile(self, **kwargs) -> Any:
        ...
        if not self._is_compiled or kwargs:
            ...
            try:
                ...
                self._app = lg_graph.compile(checkpointer=self.checkpointer, store=self.store, **kwargs)
