...
class TaskComplexity(BaseModel):
    ...
    overall_complexity: ComplexityType = Field(default=ComplexityType.SIMPLE)
...
class PlanningRequirement(BaseModel):
    ...
    planning_horizon: str = Field(default='short_term')
    constraints: list[str] = Field(default_factory=list)
    dependencies: list[str] = Field(default_factory=list)
from haive.agents.common.models.task_analysis.branching import TaskDecomposition
...
class ExecutionStrategy(BaseModel):
    ...
    strategy_type: str = Field(..., description='Primary execution approach', examples=['sequential_execution', 'parallel_execution', 'iterative_research', 'incremental_approach', 'breakthrough_research', 'collaborative_effort'])
    priority_level: str = Field(..., description='Urgency/priority level for the task', examples=['low', 'medium', 'high', 'critical', 'research_priority'])
    recommended_approach: str = Field(..., description='Detailed approach description', min_length=20, max_length=500, examples=['Execute independent branches in parallel while carefully managing dependencies', 'Conduct systematic literature review followed by expert consultation and pilot studies', 'Break into small incremental steps with validation at each stage'])
    resource_allocation: dict[str, float] = Field(..., description='Recommended resource allocation (proportions sum to 1.0)', examples=[{'computational': 0.4, 'human_expert': 0.3, 'time': 0.3}, {'research': 0.5, 'development': 0.3, 'validation': 0.2}, {'data_collection': 0.25, 'analysis': 0.35, 'interpretation': 0.4}])
    timeline_strategy: str = Field(..., description='How to manage timing and sequencing', examples=['front_load_critical_path', 'parallel_with_checkpoints', 'iterative_sprints', 'milestone_driven', 'research_then_apply', 'continuous_development'])
    risk_mitigation: list[str] = Field(..., description='Risk mitigation strategies', min_length=1, max_length=10, examples=[['backup_data_sources', 'expert_consultation', 'iterative_validation'], ['redundant_approaches', 'early_testing', 'stakeholder_engagement'], ['contingency_planning', 'resource_buffers', 'alternative_methods']])
    success_factors: list[str] = Field(..., description='Key factors for success', min_length=1, max_length=10, examples=[['clear_requirements', 'adequate_resources', 'expert_oversight'], ['stakeholder_buy_in', 'iterative_feedback', 'quality_control'], ['breakthrough_insights', 'sustained_funding', 'collaborative_environment']])
    fallback_options: list[str] = Field(default_factory=list, description='Alternative approaches if primary strategy fails', max_length=5, examples=[['simplified_version', 'manual_approach', 'external_collaboration'], ['incremental_delivery', 'proof_of_concept', 'literature_review_only'], ['theoretical_analysis', 'simulation_study', 'expert_opinion_synthesis']])
...
class TaskAnalysis(BaseModel):
    ...
    task_description: str = Field(..., description='Original task description', min_length=10, max_length=2000, examples=['Find the birthday of the most recent Wimbledon winner', "Calculate Tom Clancy's birthday + sun's age + feet in mile, then square the result", 'Develop a cure for cancer through novel therapeutic approaches', 'Prove or disprove the Riemann Hypothesis'])
    domain: str = Field(..., description='Task domain or field', examples=['sports_research', 'mathematical_calculation', 'medical_research', 'pure_mathematics', 'software_engineering', 'data_analysis', 'scientific_research', 'business_intelligence', 'creative_writing'])
    context: str | None = Field(default=None, description='Additional context about the task', max_length=1000, examples=['Factual lookup requiring web search and data verification', 'Multi-step calculation with arithmetic operations', 'Breakthrough research requiring novel scientific discoveries', 'Unsolved mathematical problem requiring proof or disproof'])
    analysis_method: AnalysisMethod = Field(default=AnalysisMethod.HYBRID, description='Method used for analysis')
    ...
    decomposition: TaskDecomposition | None = Field(default=None, description='Task decomposition (if applicable)')
    ...
    analysis_confidence: float = Field(..., description='Overall confidence in the analysis (0.0-1.0)', ge=0.0, le=1.0, examples=[0.95, 0.8, 0.6, 0.3])
    ...
    def validate_analysis_consistency(self) -> 'TaskAnalysis':
        ...
        """
        if self.complexity.overall_complexity == ComplexityType.TRIVIAL and (not self.solvability.is_currently_solvable):
            raise ValueError('Trivial complexity should be currently solvable')
        if self.complexity.overall_complexity == ComplexityType.UNSOLVABLE and self.solvability.solvability_status != SolvabilityStatus.IMPOSSIBLE:
            raise ValueError('Unsolvable complexity should have impossible solvability status')
        if self.decomposition and (self.complexity.overall_complexity == ComplexityType.TRIVIAL and len(self.decomposition.branches) > 3):
    ...
    def get_overall_assessment(self) -> dict[str, Any]:
        ...
        """
        return {'complexity_level': self.complexity.overall_complexity.value, 'complexity_score': self.complexity.get_complexity_score(), 'solvability_status': self.solvability.solvability_status.value, 'solvability_score': self.solvability.get_solvability_score(), 'is_ready_to_execute': self.solvability.is_currently_solvable, 'estimated_effort': self.complexity.estimated_steps, 'dominant_complexity_dimensions': self.complexity.get_dominant_dimensions(), 'primary_barriers': [b.value for b in self.solvability.primary_barriers], 'strategy_type': self.execution_strategy.strategy_type, 'analysis_confidence': self.analysis_confidence}
    ...
    def generate_executive_summary(self) -> str:
        ...
        summary_lines.append(f'• Ready to Execute: {('Yes' if self.solvability.is_currently_solvable else 'No')}')
        ...
        summary_lines.append(f'• Approach: {self.execution_strategy.strategy_type.replace('_', ' ').title()}')
        summary_lines.append(f'• Priority: {self.execution_strategy.priority_level.title()}')
        summary_lines.append(f'• Timeline: {self.execution_strategy.timeline_strategy.replace('_', ' ').title()}')
        summary_lines.append('')
    ...
    def get_execution_recommendations(self) -> list[str]:
        ...
        if self.complexity.overall_complexity in [ComplexityType.COMPLEX, ComplexityType.HIGHLY_COMPLEX]:
    ...
    @classmethod
    def analyze_task(cls, task_description: str, domain: str | None=None, context: str | None=None, analysis_method: AnalysisMethod=AnalysisMethod.HYBRID) -> 'TaskAnalysis':
        ...
        return cls(task_description=task_description, domain=domain, context=context, analysis_method=analysis_method, complexity=complexity, solvability=solvability, decomposition=decomposition, planning=planning, execution_strategy=execution_strategy, analysis_confidence=analysis_confidence)

    @classmethod
    def _infer_domain(cls, task_description: str) -> str:
        """Infer task domain from description."""
        text_lower = task_description.lower()
        domain_keywords = {'medical_research': ['cancer', 'cure', 'disease', 'medicine', 'therapy', 'clinical'], 'mathematics': ['prove', 'theorem', 'equation', 'calculate', 'formula', 'hypothesis'], 'sports_research': ['wimbledon', 'championship', 'tournament', 'sports', 'athlete'], 'scientific_research': ['research', 'study', 'experiment', 'analysis', 'investigation'], 'data_analysis': ['data', 'statistics', 'analyze', 'dataset', 'metrics'], 'software_engineering': ['code', 'program', 'software', 'application', 'system'], 'factual_lookup': ['find', 'birthday', 'age', 'when', 'who', 'what', 'where'], 'mathematical_calculation': ['add', 'multiply', 'square', 'sum', 'calculate']}
        domain_scores = {}
    ...
    @classmethod
    def _analyze_complexity(cls, task_description: str, domain: str, context: str | None) -> TaskComplexity:
        ...
        impossible_score = sum((1 for indicator in impossible_indicators if indicator in text_lower))
        ...
        dimension_scores = {TaskDimension.DEPTH: min(1.0, (complex_score + research_score) * 0.2), TaskDimension.BREADTH: min(1.0, len(text_lower.split()) * 0.01), TaskDimension.KNOWLEDGE: min(1.0, (complex_score + research_score) * 0.25), TaskDimension.RESEARCH: min(1.0, research_score * 0.4), TaskDimension.COORDINATION: min(1.0, complex_score * 0.15), TaskDimension.TEMPORAL: 0.3, TaskDimension.UNCERTAINTY: min(1.0, research_score * 0.3), TaskDimension.RESOURCE: min(1.0, (complex_score + research_score) * 0.2)}
        ...
        return TaskComplexity(overall_complexity=overall_complexity, dimension_scores=dimension_scores, depth_levels=depth_levels, branch_count=branch_count, estimated_steps=estimated_steps, confidence=0.8)

    @classmethod
    def _analyze_solvability(cls, task_description: str, domain: str, complexity: TaskComplexity) -> SolvabilityAssessment:
        ...
        if is_currently_solvable:
            ...
            time_to_solvable = timedelta(days=3650)
        else:
            time_to_solvable = None
        return SolvabilityAssessment(solvability_status=solvability_status, is_currently_solvable=is_currently_solvable, confidence_level=0.8, primary_barriers=primary_barriers, enabling_factors=enabling_factors, estimated_time_to_solvable=time_to_solvable, success_probability=0.9 if is_currently_solvable else 0.3)

    @classmethod
    def _generate_planning_requirements(cls, complexity: TaskComplexity, solvability: SolvabilityAssessment) -> PlanningRequirement:
        ...
        return PlanningRequirement(planning_depth=planning_depth, coordination_needs=['multi_agent'] if complexity.branch_count > 3 else [], resource_requirements=['computational', 'domain_expert'] if complexity.requires_expertise() else ['computational'], time_constraints={'real_time': False}, risk_factors=['complexity'] if complexity.overall_complexity.value in ['complex', 'highly_complex'] else [])

    @classmethod
    def _generate_execution_strategy(cls, complexity: TaskComplexity, solvability: SolvabilityAssessment, planning: PlanningRequirement) -> ExecutionStrategy:
        ...
        return ExecutionStrategy(strategy_type=strategy_type, priority_level=priority_level, recommended_approach='Execute based on complexity and solvability analysis', resource_allocation={'computational': 0.4, 'human': 0.3, 'time': 0.3}, timeline_strategy='milestone_driven', risk_mitigation=['iterative_validation', 'expert_consultation'], success_factors=['clear_requirements', 'adequate_resources'])

    @classmethod
    def _generate_decomposition(cls, task_description: str, complexity: TaskComplexity) -> TaskDecomposition | None:
        """Generate basic task decomposition."""
        if complexity.overall_complexity in [ComplexityType.TRIVIAL, ComplexityType.SIMPLE]:
            return None
        branch_descriptions = ['Initial analysis and planning', 'Core execution phase', 'Validation and refinement', 'Final integration and delivery']
        return TaskDecomposition.create_simple_sequential(task_description=task_description, branch_descriptions=branch_descriptions[:complexity.branch_count])