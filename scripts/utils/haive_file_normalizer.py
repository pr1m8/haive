...
class ImportFixer:
    ...
    def fix_relative_imports(self, source_code: str, file_path: Path) -> str:
        ...
        if not LIBCST_AVAILABLE:
            self.console.print("[red]libcst not available. Install with: pip install libcst[/red]")
            return source_code
...
class RelativeImportTransformer(cst.CSTTransformer):
    ...
    def leave_ImportFrom(self, original_node: cst.ImportFrom, updated_node: cst.ImportFrom) -> cst.ImportFrom:
        ...
        if isinstance(updated_node.module, cst.Attribute) or (
            isinstance(updated_node.module, cst.Name) and 
            str(updated_node.module.value).startswith('.')
        ):
            # Convert relative to absolute
            relative_path = str(updated_node.module.value) if hasattr(updated_node.module, 'value') else str(updated_node.module)
            ...
            if relative_path.startswith('.'):
                ...
                if self.current_module:
                    ...
                    if levels == 1:  # from .module
                        ...
                        base_module = '.'.join(module_parts[:-(levels-1)] if levels > 1 else module_parts)
                    ...
                    return updated_node.with_changes(module=cst.Attribute(
                        value=cst.Name(absolute_import.split('.')[0]),
                        attr=cst.Name('.'.join(absolute_import.split('.')[1:]))
                    ) if '.' in absolute_import else cst.Name(absolute_import))
...
class FileNormalizer:
    ...
    def find_common_prefix(self, files: List[str]) -> str:
        ...
        dir_name = self.directory.name.lower().replace(" ", "").replace("-", "").replace("_", "")
    ...
    def normalize_filename(self, filename: str, common_prefix: str) -> str:
        ...
        name_part = re.sub(r'^_+|_+$', '', name_part)     # Remove leading/trailing underscores
    ...
    def normalize_directory(self, dry_run: bool = True) -> Dict[str, any]:
        ...
        if common_prefix:
            self.console.print(f"[green]Detected common prefix:[/green] '{common_prefix}'")
            ...
            for base_name, files in file_groups.items():
                ...
                for file in files_to_process:
                    ...
                    if len(file_groups[base_name]) > 1:
                        ...
                        normalized_name = self.normalize_filename(base_name, common_prefix)
                    else:
                        # Single file, normalize as normal
                        normalized_name = self.normalize_filename(file.name, common_prefix)
                    ...
                    if normalized_name != file.name:
                        ...
                        while new_path.exists() and new_path != file:
                            name_part, ext = original_normalized.rsplit('.', 1) if '.' in original_normalized else (original_normalized, '')
                            normalized_name = f"{name_part}_{counter}.{ext}" if ext else f"{name_part}_{counter}"
                            new_path = file.parent / normalized_name
                            counter += 1
    ...
    def _display_results(self, changes: List[Dict], duplicates: List[Dict], dry_run: bool):
        """Display the results of normalization."""
        if not changes and not duplicates:
            self.console.print("[green]✓ No changes needed - all files are already normalized[/green]")
            return
        
        # Create results table
        table = Table(title=f"File Normalization Results {'(DRY RUN)' if dry_run else ''}")
...
class DownloadNormalizer:
    ...
    def normalize_downloads(self, pattern: str, destination: Path, limit: int = 10, dry_run: bool = True) -> Dict[str, any]:
        """Find, normalize, and move matching downloads."""
        matching_files = self.find_matching_downloads(pattern, limit)
        
        if not matching_files:
            self.console.print(f"[yellow]No Python files found matching pattern '{pattern}'[/yellow]")
        ...
        try:
            ...
            if temp_dir:
                normalizer = FileNormalizer(temp_dir if not dry_run else self.downloads_dir)
...
Examples:
  ...
  python file_normalizer.py all ./my_files/ --target-package haive.games.holdem --execute
    ...
    normalize_parser.add_argument('--execute', action='store_true', help='Execute changes (default is dry run)')
    ...
    fix_parser.add_argument('--target-package', required=True, help='Target package (e.g., haive.games.holdem)')
    fix_parser.add_argument('--execute', action='store_true', help='Execute changes (default is dry run)')
    
    # Download normalize command
    download_parser = subparsers.add_parser('download-normalize', help='Normalize downloads')
    download_parser.add_argument('pattern', help='Pattern to match in download filenames')
    download_parser.add_argument('destination', type=Path, help='Destination directory')
    download_parser.add_argument('--limit', type=int, default=10, help='Max files to process')
    download_parser.add_argument('--execute', action='store_true', help='Execute changes (default is dry run)')
    
    # All command
    all_parser = subparsers.add_parser('all', help='Run normalization and import fixing')
    ...
    all_parser.add_argument('--execute', action='store_true', help='Execute changes (default is dry run)')
    ...
    try:
        if args.command == 'normalize':
            ...
            if not LIBCST_AVAILABLE:
                console.print("[red]libcst is required for import fixing. Install with: pip install libcst[/red]")
                return 1
            ...
            with Progress(console=console) as progress:
                ...
                for py_file in python_files:
                    try:
                        ...
                        fixed_content = import_fixer.fix_relative_imports(original_content, py_file)
            ...
            if args.target_package and LIBCST_AVAILABLE:
                ...
                console.print("[yellow]Skipping import fixing - libcst not available[/yellow]")
