...
class ReadmeToDocstringConverter:
    ...
    def _find_corresponding_init(self, readme_path: Path) -> Path | None:
    ...
    def _determine_module_type(self, readme_path: Path, content: str) -> str:
        ...
        if "agent" in path_str:
            if "base" in path_str:
                return "base_module"
            if any(x in path_str for x in ["rag", "react", "planning", "reasoning"]):
                return "agent_implementation"
            return "agent_module"
        if "engine" in path_str or "core" in path_str:
            return "core_module"
        if "tool" in path_str:
            return "tool_module"
    ...
    def create_conversion_plan(self) -> None:
        ...
        for module_type, _count in sorted(type_counts.items()):
            pass
...
def main():
    ...
    report_file = (
        Path(__file__).parent.parent / "project_docs" / "readme_conversion_plan.md"
    )
