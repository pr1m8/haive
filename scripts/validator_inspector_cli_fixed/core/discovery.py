...
class ValidatorInspector(cst.CSTVisitor):
    ...
    def _check_model_validator(self, node: cst.FunctionDef,
                               decorator: cst.Call):
        """Check model_validator for issues."""
        # Extract mode parameter
        mode_arg = next(
            (arg for arg in decorator.args
        ...
        uses_self = any(param.name.value == 'self'
                        for param in node.params.params)
        has_classmethod = any(
            m.matches(d.decorator, m.Name('classmethod'))
            for d in node.decorators)
        return_annot = node.returns.annotation if node.returns else None

        # Determine mode (default is "before" if not specified)
        mode = 'before'  # default
        if mode_arg and hasattr(mode_arg, 'value') and 'after' in str(
                mode_arg.value):
            mode = 'after'
    ...
    def _check_field_validator(self, node: cst.FunctionDef,
                               decorator: cst.Call):
        ...
        return_annot = node.returns.annotation if node.returns else None
        if return_annot and hasattr(return_annot,
                                    'code') and 'Self' in return_annot.code:
            self.issues.append((
                node.name.value,
