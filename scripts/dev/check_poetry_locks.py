...
def main():
    ...
    for package in packages:
        ...
        if not is_sync:
            print(f"❌ {package} lock file is out of sync:")
            print(error)
            all_good = False
        else:
            print(f"✅ {package} lock file is in sync")
    
    if not all_good:
        print("\nSome lock files are out of sync. Run `poetry lock` in each package directory.")
        sys.exit(1)
    
    print("\nAll lock files are in sync!")
