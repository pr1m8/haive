...
create_safety_checkpoint() {
	...
	echo -e "${GREEN}🛡️ Safety checkpoint created${for $dire}ctory: REFACTOR_CHECKPOINT_${desc}_${TIMESTAMP}${NC}"
}
...
bowler_rename_function() {
	...
	echo -e "${BLUE}🔄 Bowler renam${: $old_n}ame${→ $new}_name${NC}"
...
"extract-method")
	...
	echo -e "${CYAN}🔄 EXTRACT METHOD: Lin${s $START_L}I${E-$END_L}INE${→ $METHOD}_NAME()${NC}"
	echo -e "${YELLOW}📁 Fil${: $F}ILE${NC}"
...
"find-usages")
	...
	echo -e "${CYAN}🔍 FIND USAGE${: $SYM}BOL${NC}"
	echo -e "${YELLOW}📁 Director${: $DIRECT}ORY${NC}"
	echo ""
...
*)
	echo -e "${RED}❌ Unknown operation${ $OPERATI}ON${NC}"
