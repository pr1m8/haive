...
echo "  🔴 'retur ' (missing 'n'${: $RETUR_FI}LES occurrences"
echo "  🔴 'retur' (line ending${: $RETUR_ONLY_FI}LES occurrences"
echo "  🔴 ') -> st:' (should be str${: $ST_TYPE_FI}LES occurrences"
echo "  🔴 Unterminated string${: $UNTERMINATED_STRI}NGS files"
echo ""
...
if [[ "${MODE}" = "--fix" ]]; then
	...
	if [[ "${RETUR_FILES}" -gt 0 ]]; then
		...
		echo "  ✅ Fixe${ $RETUR_FIL}ES occurrences"
	fi
	...
	if [[ "${RETUR_ONLY_FILES}" -gt 0 ]]; then
		...
		find "${DIRECTORY}" -name "*.py" -type f -exec sed -i.bak 's/return/return/g' {} \;
		echo "  ✅ Fixe${ $RETUR_ONLY_FIL}ES occurrences"
	fi
	...
	if [[ "${ST_TYPE_FILES}" -gt 0 ]]; then
		...
		echo "  ✅ Fixe${ $ST_TYPE_FIL}ES occurrences"
	fi
...
")
	...
	echo "  ✅ Files that now compile${ $COMPILABLE_FIL}ES"
	echo "  🔴 Remaining IndentationError${: $NEW_INDENT_ERR}ORS"
	echo "  🔴 Remaining SyntaxError${: $NEW_SYNTAX_ERR}ORS"
