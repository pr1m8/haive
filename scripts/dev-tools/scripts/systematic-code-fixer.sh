...
echo "  📁 Total Python file${: $TOTAL_FI}LES"
echo "  🚨 Files with SyntaxError${: $INITIAL_SYNTAX_ERR}ORS"
echo "  🔧 Files with IndentationError${: $INITIAL_INDENT_ERR}ORS"
...
if [[ "${STEP_NUM}" = "all" ]] || [[ "${STEP_NUM}" = "0" ]]; then
	...
	if [[ "${MODE}" = "--fix" ]]; then
		...
		echo -e "${BLUE}👀 Preview: Would run autoimport to f${x $F821_CO}UNT undefined names (F821)${NC}"
		echo ""
	fi
fi
...
")
		...
		echo "  🚨 SyntaxError${: $INITIAL_SYNTAX_ERR}ORS${→ $STEP1_S}YNTAX (fixed: $((INITIAL_SYNTAX_ERRORS - STEP1_SYNTAX)))"
		echo "  🔧 IndentationError${: $INITIAL_INDENT_ERR}ORS${→ $STEP1_I}NDENT"
		echo ""

		if [[ "${STEP1_SYNTAX}" -gt 0 ]]; then
			echo -e "${YELLOW}⚠️  Still h${ve $STEP1_SY}NTAX syntax errors - may need manual fixes${NC}"
			echo ""
		fi
	else
...
if [[ "${STEP_NUM}" = "all" ]] || [[ "${STEP_NUM}" = "3" ]]; then
	...
	if [[ "${MODE}" = "--fix" ]]; then
		...
		F401_COUNT=$(poetry run ruff check "${DIRECTORY}" --select F401 2>/dev/null | wc -l || echo "0")
		I001_COUNT=$(poetry run ruff check "${DIRECTORY}" --select I001 2>/dev/null | wc -l || echo "0")
		echo -e "${BLUE}👀 Preview: Would f${x $F401_CO}UNT unused imports a${d $I001_CO}UNT unsorted imports${NC}"
		echo ""
	fi
fi
...
if [[ "${STEP_NUM}" = "all" ]] || [[ "${STEP_NUM}" = "4" ]]; then
	...
	if [[ "${MODE}" = "--fix" ]]; then
		...
		UP_COUNT=$(poetry run ruff check "${DIRECTORY}" --select UP 2>/dev/null | wc -l || echo "0")
		DTZ_COUNT=$(poetry run ruff check "${DIRECTORY}" --select DTZ 2>/dev/null | wc -l || echo "0")
		echo -e "${BLUE}👀 Preview: Would moderni${e $UP_CO}UNT syntax upgrades a${d $DTZ_CO}UNT datetime issues${NC}"
		echo ""
	fi
fi
...
")
	...
	echo "  🚨 SyntaxError${: $INITIAL_SYNTAX_ERR}ORS${→ $FINAL_S}YNTAX"
	echo "  🔧 IndentationError${: $INITIAL_INDENT_ERR}ORS${→ $FINAL_I}NDENT"
	echo "  ✅ Clean files: $((TOTAL_FILES - INITIAL_SYNTAX_ERRORS - INITIAL_INDENT_ERRORS)) ${�� $CLEAN_F}ILES"
	echo ""
	echo -e "${YELLOW}📊 RUFF ERRORS BEFORE → AFTER:${NC}"
	echo "  🔴 F821 (undefined names): (baseline)${→ $FINAL}_F821"
	echo "  🔴 F401 (unused imports): (baseline)${→ $FINAL}_F401"
	echo "  🔴 I001 (unsorted imports): (baseline)${→ $FINAL}_I001"
	echo "  📊 Total ruff errors: (baseline)${→ $FINAL_TOTAL_E}RRORS"
	echo ""
	...
	if [[ "${SYNTAX_FIXED}" -gt 0 ]]; then
		echo -e "${GREEN}✅ Fixe${ $SYNTAX_FIX}ED syntax errors${NC}"
	fi
	if [[ "${INDENT_FIXED}" -gt 0 ]]; then
		echo -e "${GREEN}✅ Fixe${ $INDENT_FIX}ED indentation errors${NC}"
	fi
	...
	if [[ "${FINAL_SYNTAX}" -eq 0 ]] && [[ "${FINAL_INDENT}" -eq 0 ]]; then
		...
		[[ "${FINAL_SYNTAX}" -gt 0 ]] && echo "   �${� $FINAL_SYN}TAX files with syntax errors"
		[[ "${FINAL_INDENT}" -gt 0 ]] && echo "   �${� $FINAL_IND}ENT files with indentation errors"
	fi
