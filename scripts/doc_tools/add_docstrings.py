...
def generate_function_docstring(
    ...
    if func_name.startswith("get_"):
        ...
        desc = f"Check if the game has reached a win condition."
    elif func_name == "evaluate_position":
        desc = f"Evaluate the current game position strategically."
    elif func_name == "update_state":
        desc = f"Update the game state based on the latest move or changes."
    else:
...
def create_module_readme(module_path: str, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        try:
                ...
                if filename.endswith(".py") and filename != "__init__.py":
                    ...
                    if base_name in [
                        ...
                        if base_name == "models":
                            ...
                            try:
                                with open(
                                    os.path.join(module_path, filename), "r"
                                ) as f:
                                    content = f.read()
            ...
            print(f"Warning: Error analyzing components in {module_path}: {e}")
            # Default component if analysis fails
            components = [f"{display_name}Component"]
...
"""
        ...
        print(f"Error processing README for {module_path}: {e}")
        return False
