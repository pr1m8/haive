...
def cmd_demo(args):
    ...
    try:
        ...
        class DemoAgent:
            ...
            def stream(self, input_data, **kwargs):
                steps = [
                    ...
                    {
                        "step": "output",
                        "content": {"result": f"Processed: {input_data}"},
                    },
                ]
                yield from steps
        ...
        run = capture_agent_run(
            agent, example_input, agent_name="DemoAgent", capture_dir="docs/captures"
        )
...
def cmd_capture(args):
    ...
    try:
        ...
        # Parse input
        input_data = json.loads(args.input) if args.input.startswith("{") else {"query": args.input}
...
def cmd_document(args):
    ...
    try:
        ...
        AgentDocumentationGenerator(docs_dir=args.docs_dir)
...
Examples:
  ...
  poetry run python scripts/agent_capture_cli.py list
    ...
    capture_parser.add_argument(
        "--input", required=True, help="Input data (JSON string or plain text)"
    )
    capture_parser.add_argument(
        "--output-dir", default="docs/captures", help="Output directory"
    )
    ...
    doc_parser.add_argument(
        "--docs-dir", default="docs/source", help="Documentation directory"
    )
    ...
    list_parser.add_argument(
        "--capture-dir", default="docs/captures", help="Capture directory"
    )
