...
def extract_script_from_patch(patch_content, script_name):
    ...
    if not match:
        print(f"❌ Could not find {script_name} in patch")
        return None
...
def main():
    ...
    for script_name in priority_scripts:
        ...
        if script_path.exists():
            print(f"⏭️  {script_name} already exists, skipping")
            continue
            
        print(f"🔧 Extracting {script_name}...")
        ...
        if script_content:
            ...
            print(f"✅ Successfully extracted {script_name}")
            successful_extractions += 1
        else:
            print(f"❌ Failed to extract {script_name}")
    
    print(f"\n🎉 Successfully extracted {successful_extractions} scripts!")
    print("📁 Scripts available in dev-tools/scripts/")
