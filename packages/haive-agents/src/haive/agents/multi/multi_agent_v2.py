...
class MultiAgentV2(Agent[MultiAgentState]):
    ...
    execution_mode: ExecutionMode = Field(
        default=ExecutionMode.SEQUENCE,
    ...
    routing_function: Optional[Callable] = Field(
        default=None,
        description="Function for conditional routing"
    )
    
    route_map: Optional[Dict[str, str]] = Field(
        default=None,
        description="Map of routing outputs to agent names"
    )
    ...
    def _build_sequence_graph(self, graph: BaseGraph) -> None:
        ...
        agent_names = list(self.agents.keys()) if isinstance(self.agents, dict) else [a.name for a in self.agents]
