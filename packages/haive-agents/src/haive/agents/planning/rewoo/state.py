...
class ReWOOState(ToolState):
    ...
    plan: Optional[ReWOOPlan] = Field(
        default=None,
        description="Current ReWOO plan"
    )
    ...
    evidence_map: Dict[str, Evidence] = Field(
        ...
        description="Map of evidence ID to evidence objects"
    )
    
    evidence_collection_order: List[str] = Field(
        default_factory=list,
        description="Order in which evidence was collected"
    )
    
    failed_evidence_attempts: Dict[str, List[str]] = Field(
        default_factory=dict,
        description="Map of evidence ID to error messages"
    )
    ...
    active_tool_calls: List[ToolCall] = Field(
        default_factory=list,
        description="Currently executing tool calls"
    )
    
    # Evidence-specific tool mapping
    evidence_tool_mapping: Dict[str, str] = Field(
        default_factory=dict,
        description="Map evidence IDs to specific tool names"
    )
    ...
    final_reasoning: Optional[ReWOOReasoning] = Field(
        default=None,
        description="Final structured reasoning output"
    )
    ...
    max_evidence_collection_rounds: int = Field(
        default=10,
        description="Maximum rounds of evidence collection"
    )
    
    current_collection_round: int = Field(
        default=0,
        description="Current evidence collection round"
    )
    
    allow_parallel_collection: bool = Field(
        default=True,
        description="Whether to collect evidence in parallel"
    )
    
    skip_failed_evidence: bool = Field(
        default=False,
        description="Whether to skip failed evidence and continue"
    )
    ...
    def collected_evidence_ids(self) -> Set[str]:
        ...
        return {
            eid for eid, evidence in self.evidence_map.items()
    ...
    def ready_evidence(self) -> List[Evidence]:
        ...
        for evidence in self.evidence_map.values():
            ...
            # Check dependencies
            deps_met = all(
                dep_id in self.collected_evidence_ids
    ...
    def is_evidence_complete(self) -> bool:
        ...
        for evidence in self.evidence_map.values():
            if evidence.status == EvidenceStatus.PLANNED:
                return False
            if evidence.status == EvidenceStatus.FAILED and not self.skip_failed_evidence:
                return False
...
class ReWOOStateWithRouting(ReWOOState):
    ...
    tool_route_overrides: Dict[str, str] = Field(
        default_factory=dict,
        description="Override routes for specific tools"
    )
    
    failed_tool_fallbacks: Dict[str, List[str]] = Field(
        default_factory=dict,
        description="Fallback tools for failed executions"
    )
