...
class Evidence(BaseModel):
    ...
    id: str = Field(
        ...,
        description="Evidence identifier (e.g., #E1, #E2)",
        pattern=r"^#E\d+$"
    )
    ...
    evidence_type: EvidenceType = Field(
        default=EvidenceType.TOOL_OUTPUT,
    ...
    source: str = Field(
        ...,
        description="Source of evidence (tool name, LLM, etc.)"
    )
    
    collection_method: str = Field(
        ...,
        description="How to collect this evidence"
    )
    
    # Dependencies on other evidence
    depends_on: List[str] = Field(
        default_factory=list,
        description="Other evidence IDs this depends on"
    )
    
    # Status tracking
    status: EvidenceStatus = Field(
        default=EvidenceStatus.PLANNED,
    ...
    error: Optional[str] = Field(
        default=None,
        description="Error if collection failed"
    )
...
class ToolCall(BaseModel):
    ...
    tool_name: str = Field(
        ...,
        description="Name of tool or 'LLM' for reasoning"
    )
    
    tool_type: ToolCallType = Field(
        default=ToolCallType.STANDARD,
    ...
    arguments: Dict[str, Any] = Field(
        ...
        description="Tool arguments (can include #E references)"
    )
    
    # Expected output
    expected_output_type: Optional[str] = Field(
        default=None,
        description="Expected type of output (for validation)"
    )
    
    expected_output_schema: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Expected schema for structured output"
    )
...
class ReWOOStep(BaseStep):
    ...
    step_type: StepType = Field(
        default=StepType.EVIDENCE,
        description="ReWOO steps are evidence-based"
    )
    
    # Evidence this step produces
    evidence: Optional[Evidence] = Field(
        default=None,
        description="Evidence this step produces"
    )
    
    # Tool call for this step
    tool_call: Optional[ToolCall] = Field(
        default=None,
        description="Tool call to execute"
    )
    
    # Alternative: Multiple tool calls with fallback
    tool_calls: List[ToolCall] = Field(
        default_factory=list,
        description="Multiple tool calls (fallback options)"
    )
    
    # How to process tool output into evidence
    output_processor: Optional[str] = Field(
        default=None,
        description="Python expression to process tool output"
    )
...
class ReWOOPlan(BasePlan):
    ...
    steps: List[ReWOOStep] = Field(
        default_factory=list,
        description="ReWOO steps in the plan"
    )
    
    # Evidence registry
    evidence_map: Dict[str, Evidence] = Field(
        default_factory=dict,
        description="Map of evidence ID to evidence"
    )
    ...
    parallel_evidence_collection: bool = Field(
        default=True,
        description="Whether to collect independent evidence in parallel"
    )
    
    max_parallel_collections: int = Field(
        default=5,
        description="Maximum parallel evidence collections",
        ge=1
    )
    ...
    def next_evidence_to_collect(self) -> List[Evidence]:
        ...
        for evidence in self.evidence_map.values():
            ...
            deps_satisfied = all(
                self.evidence_map.get(dep_id, Evidence(id=dep_id, description="", source="")).status == EvidenceStatus.COLLECTED
    ...
    def evidence_completion_percentage(self) -> float:
        ...
        collected = sum(
            1 for e in self.evidence_map.values()
    ...
    def update_evidence_status(
        ...
        for step in self.steps:
            if isinstance(step, ReWOOStep) and step.evidence and step.evidence.id == evidence_id:
    ...
    def add_rewoo_step(
        ...
        for dep_id in depends_on or []:
            ...
            for existing_step in self.steps:
                if isinstance(existing_step, ReWOOStep) and existing_step.evidence_id == dep_id:
                    step.add_dependency(
                        existing_step.id,
                        DependencyType.DATA,
                        required_output=dep_id
                    )
...
class ReWOOReasoning(BaseModel):
    ...
    objective_restatement: str = Field(
        ...,
        description="Restatement of the objective"
    )
    ...
    conclusion: str = Field(
        ...,
        description="Final conclusion based on evidence"
    )
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence in conclusion"
    )
    ...
    next_steps: Optional[List[str]] = Field(
        default=None,
        description="Suggested next steps if needed"
    )