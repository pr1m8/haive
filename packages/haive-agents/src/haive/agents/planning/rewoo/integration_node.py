...
from haive.agents.planning.rewoo.models import ToolCall, EvidenceStatus
...
async def tool_results_to_evidence_node(state: ReWOOState) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "messages": [f"Collected evidence {current_evidence_id}: {tool_message.content[:100]}..."]
        }
...
def build_rewoo_integration_graph():
    ...
    from haive.core.graph import BaseGraph, START, END
