...
class ReWOOPlanningNodeConfig(BaseNodeConfig[ReWOOPlanningInput, ReWOOPlanningOutput]):
    ...
    def get_default_output_fields(self) -> List[FieldDefinition]:
        ...
        return [
            FieldDefinition(
                name="plan",
                field_type=ReWOOPlan,
                description="Generated ReWOO plan"
            ),
            FieldDefinition(
                name="evidence_map",
    ...
    def __call__(self, state: StateLike, config: Optional[ConfigLike] = None) -> Command:
...
class ReWOOEvidenceNodeConfig(BaseNodeConfig[ReWOOEvidenceInput, ReWOOEvidenceOutput]):
    ...
    def __call__(self, state: StateLike, config: Optional[ConfigLike] = None) -> Command:
        ...
        for evidence in evidence_map.values():
            ...
            # Check dependencies
            deps_met = all(
                evidence_map.get(dep_id, Evidence(id=dep_id, description="", source="")).status == EvidenceStatus.COLLECTED
        ...
        if not step or not step.tool_call:
            return Command(update={"messages": [f"No tool call for evidence {evidence.id}"]})
...
class ReWOOReasoningNodeConfig(BaseNodeConfig[ReWOOReasoningInput, ReWOOReasoningOutput]):
...
def create_rewoo_planning_node(
    name: str = "rewoo_planning",
    engine_name: str = "aug_llm",
    **kwargs
) -> ReWOOPlanningNodeConfig:
    """Create ReWOO planning node config."""
    return ReWOOPlanningNodeConfig(
        name=name,
