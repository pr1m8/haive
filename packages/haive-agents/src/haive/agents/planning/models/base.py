...
class Dependency(BaseModel):
    ...
    required_output: Optional[str] = Field(
        default=None,
        description="Specific output field required from dependency"
    )
...
class BaseStep(BaseModel):
    ...
    dependencies: List[Dependency] = Field(
        ...
        description="Steps this depends on"
    )
...
class ActionStep(BaseStep):
    ...
    expected_output_schema: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Expected structure of output"
    )
...
class RecursiveStep(BaseStep):
    ...
    sub_objective: str = Field(
        ...,
        description="Objective for sub-plan"
    )
    ...
    sub_plan_id: Optional[str] = Field(
        default=None,
        description="ID of generated sub-plan"
    )
...
class ConditionalStep(BaseStep):
    ...
    then_steps: List[str] = Field(
        default_factory=list,
        description="Steps to execute if true"
    )
    
    else_steps: List[str] = Field(
        default_factory=list,
        description="Steps to execute if false"
    )
...
class ParallelStep(BaseStep):
    ...
    parallel_steps: List[str] = Field(
        default_factory=list,
        description="IDs of steps to run in parallel"
    )
    
    join_strategy: str = Field(
        default="all",
        description="How to join results: 'all', 'any', 'majority'"
    )


# Type alias for any step type
AnyStep = Union[BaseStep, ActionStep, ResearchStep, RecursiveStep, 
...
class BasePlan(BaseModel):
    ...
    objective: str = Field(
        ...,
        description="Main objective/goal"
    )
...
class AdaptivePlan(BasePlan):
    ...
    max_adaptations: int = Field(
        default=3,
        description="Maximum adaptation cycles"
    )
    
    adaptation_count: int = Field(
        default=0,
        description="Current adaptation count"
    )
