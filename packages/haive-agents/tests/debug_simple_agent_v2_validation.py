...
RAG_QUERY_REFINEMENT = ChatPromptTemplate.from_messages([
    ("system", """You are an expert query optimization specialist for RAG systems. Your role is to analyze user queries and suggest improvements that will lead to better document retrieval and more accurate answers.
...
def inspect_pydantic_model(model_class: type, name: str) -> None:
    ...
    if hasattr(model_class, 'model_fields'):
        ...
        for field_name, field_info in fields.items():
            ...
            is_required = field_info.is_required() if hasattr(field_info, 'is_required') else (field_info.default is ...)
...
def test_debug_simple_agent_v2_validation():
    ...
    debug_step("STEP 1: Prompt Template Analysis", RAG_QUERY_REFINEMENT, 
               "langchain_core.prompts.ChatPromptTemplate")
    
    print(f"\n🔍 Prompt Template Details:")
    print(f"📋 Input Variables: {RAG_QUERY_REFINEMENT.input_variables}")
    print(f"📋 Partial Variables: {getattr(RAG_QUERY_REFINEMENT, 'partial_variables', {})}")
    ...
    debug_step("AugLLMConfig Created", config, 
    ...
    try:
        ...
        try:
            ...
            print(f"   ✅ SchemaComposer imported successfully")
            ...
            try:
                ...
                inspect_pydantic_model(MessagesStateWithTokenUsage, "MessagesStateWithTokenUsage (Base)")
            except Exception as e:
        ...
        if hasattr(config, 'derive_input_schema'):
            try:
                ...
                print(f"   ✅ Engine Input Schema Derived:")
        ...
        if hasattr(config, 'derive_output_schema'):
            try:
                ...
                print(f"   ✅ Engine Output Schema Derived:")
        ...
        debug_step("SimpleAgentV2 Created", agent,
        ...
        if hasattr(agent, 'state_schema'):
            ...
            if hasattr(agent, '_schema_composer'):
                ...
                debug_step("Schema Composer Instance", composer,
                ...
                print(f"   ❌ No _schema_composer found on agent")
        else:
            print(f"   ❌ No state_schema found on agent")
    ...
    if hasattr(agent, 'build_graph'):
        ...
        try:
            ...
            if hasattr(graph, 'nodes'):
                print(f"   🔸 Graph Nodes: {list(graph.nodes.keys()) if hasattr(graph.nodes, 'keys') else graph.nodes}")
    ...
    if hasattr(agent, '_app') and agent._app:
        ...
        if hasattr(app, 'input_model') and app.input_model:
            ...
            print(f"   🔸 Input Model Module: {getattr(app.input_model, '__module__', 'UNKNOWN')}")
            print(f"   🔸 Input Model Name: {getattr(app.input_model, '__name__', 'UNKNOWN')}")
            ...
            input_fields = app.input_model.model_fields if hasattr(app.input_model, 'model_fields') else {}
            ...
            for critical_field in ['context', 'engine']:
                ...
                if critical_field in state_fields:
                    ...
                    state_required = state_field.is_required() if hasattr(state_field, 'is_required') else (state_field.default is ...)
                    print(f"  📊 State Schema: required={state_required}, default={getattr(state_field, 'default', 'NO_DEFAULT')}")
                else:
                    print(f"  📊 State Schema: NOT PRESENT")
                
                if critical_field in input_fields:
                    input_field = input_fields[critical_field]
                    input_required = input_field.is_required() if hasattr(input_field, 'is_required') else (input_field.default is ...)
                    print(f"  📊 Input Model: required={input_required}, default={getattr(input_field, 'default', 'NO_DEFAULT')}")
                    
                    if critical_field in state_fields:
                        state_field = state_fields[critical_field]
                        state_required = state_field.is_required() if hasattr(state_field, 'is_required') else (state_field.default is ...)
                        if state_required != input_required:
                            print(f"  🚨 MISMATCH DETECTED! State={state_required}, Input={input_required}")
                            print(f"      This is likely the source of the validation error!")
                else:
                    print(f"  📊 Input Model: NOT PRESENT")
                    
            # Check if we can trace the input model creation
            print(f"\n🔍 Tracing Input Model Creation:")
            print(f"   This input_model was likely created during graph.compile()")
            print(f"   Location: langgraph/pregel/__init__.py or BaseGraph.compile()")
    ...
    try:
        ...
        if hasattr(registry, 'engines'):
            ...
            if engine_name and engine_name in registry.engines:
                ...
                debug_step(f"Engine Registry Info: {engine_name}", engine_info, 
    ...
    try:
        ...
        if "validation errors" in str(e):
            ...
            print(f"This confirms the issue is in Pydantic validation during LangGraph execution.")
            print(f"The error occurs when LangGraph tries to validate the input using input_model.")
