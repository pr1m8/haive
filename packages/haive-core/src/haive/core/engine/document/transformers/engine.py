...
class DocTransformerConfig(BaseModel):
    ...
    ignore_links: bool = Field(default=True, description="Whether to ignore links in HTML")
    ignore_images: bool = Field(default=True, description="Whether to ignore images in HTML")
    ...
    remove_comments: bool = Field(default=False, description="Whether to remove comments from HTML")
    
    # Markdown options
    heading_style: str = Field(default="ATX", description="Heading style for markdown conversion")
    autolinks: bool = Field(default=True, description="Whether to use automatic link style")
    ...
    preserve_hierarchy: bool = Field(
        default=True, description="Whether to preserve document hierarchy during transformation"
    )
        ...
        try:
            ...
            for doc_index, document in enumerate(documents_to_transform):
                ...
                for transform_index, transformed_doc in enumerate(transformed_docs):
                    ...
                    original_doc_id = document.metadata.get("document_id") or f"doc_{doc_index}_{int(start_time)}"
                    ...
                    enhanced_metadata = {
                        **transformed_doc.metadata,
                        ...
                        "is_child_document": document.metadata.get("is_child_document", False),
                        "document_hierarchy_level": document.metadata.get("document_hierarchy_level", 0),
                        ...
                        "original_document_metadata": {
                            ...
                            "original_document_type": document.metadata.get("document_type", "unknown"),
                        ...
                        "length_change": len(transformed_doc.page_content) - len(document.page_content),
        ...
        try:
            if transformer_type == DocTransformerType.HTML_TO_TEXT:
                ...
                if self.config.embeddings_model is None:
                    raise ValueError("Embeddings model is required for EmbeddingsRedundantFilter")
                ...
                if self.config.embeddings_model is None:
                    raise ValueError("Embeddings model is required for EmbeddingsClusteringFilter")
                return EmbeddingsClusteringFilter(embeddings=self.config.embeddings_model)
                ...
                return GoogleTranslateTransformer(target_language=self.config.target_language)
            ...
            raise ValueError(f"Required package not installed for {transformer_type}: {e}")
        except Exception as e:
            raise ValueError(f"Failed to create transformer: {e}")
    
    @staticmethod
    def get_transformed_docs(document_state: DocumentState, original_id: str) -> List[Document]:
        ...
        """
        return [
            doc for doc in document_state.raw_documents 
            if doc.metadata.get("original_document_id") == original_id
        ]
    
    @staticmethod
    def get_original_doc(document_state: DocumentState, transform_id: str) -> Optional[Document]:
