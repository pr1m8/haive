...
class DocSplitterInputSchema(BaseModel):
    ...
    # Advanced options
    length_function: str = Field(default="len", description="Function to measure chunk length")
...
class DocSplitterOutputSchema(BaseModel):
    ...
    original_documents: int = Field(..., description="Number of original input documents")
    ...
    splitter_config: Dict[str, Any] = Field(..., description="Splitter configuration used")
...
class DocSplitterConfig(BaseModel):
    ...
    splitter_type: DocSplitterType = Field(
        default=DocSplitterType.RECURSIVE_CHARACTER,
        description="Default splitter type"
    )
    ...
    strip_whitespace: bool = Field(default=True, description="Strip whitespace by default")
    add_start_index: bool = Field(default=True, description="Add start index by default")
...
class DocumentSplitterEngine(InvokableEngine[DocumentState, DocumentState]):
        ...
        try:
            ...
            for doc_index, document in enumerate(documents_to_split):
                ...
                for chunk_index, chunk in enumerate(chunks):
                    ...
                    parent_doc_id = document.metadata.get("document_id") or f"doc_{doc_index}_{int(start_time)}"
                    ...
                    enhanced_metadata = {
                        **chunk.metadata,
                        ...
                        "document_hierarchy_level": (document.metadata.get("document_hierarchy_level", 0) + 1),
                        ...
                        "sibling_chunk_ids": [f"{parent_doc_id}_chunk_{i}" for i in range(len(chunks))],
                        
                        # Parent document information
                        "parent_document_metadata": {
                            "source": document.metadata.get("source", "unknown"),
                            "original_length": len(document.page_content),
                            "parent_document_type": document.metadata.get("document_type", "unknown"),
                        },
                        ...
                        "chunk_position_in_parent": {
                            ...
                            "end_char": getattr(chunk.metadata, 'start_index', 0) + len(chunk.page_content) if hasattr(chunk.metadata, 'start_index') else None,
                        },
                    }
    ...
    @staticmethod
    def get_children_docs(document_state: DocumentState, parent_id: str) -> List[Document]:
    ...
    @staticmethod
    def get_parent_doc(document_state: DocumentState, child_id: str) -> Optional[Document]:
    ...
    @staticmethod
    def get_sibling_docs(document_state: DocumentState, chunk_id: str) -> List[Document]:
