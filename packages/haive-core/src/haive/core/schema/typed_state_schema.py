...
class TypedStateSchema(StateSchema, Generic[TEngine]):
    ...
    engine: Optional[Union[TEngine, Dict[str, Any]]] = Field(
        default=None,
        description="Optional main/primary engine (typed)"
    )
    ...
    def __class_getitem__(cls, engine_type: Type[TEngine]) -> Type["TypedStateSchema"]:
        ...
        class _TypedSchema(cls):
            ...
            engine: Optional[Union[engine_type, Dict[str, Any]]] = Field(
                default=None,
                description=f"Optional {engine_type.__name__} engine"
            )
...
class MultiEngineStateSchema(StateSchema):
    ...
    engines: Dict[str, Union["Engine", Dict[str, Any]]] = Field(
        ...
        description="Registry of named engines with proper typing"
    )
    ...
    def register_engine(
        ...
        name: str, 
        engine: "Engine",
        engine_type: Optional[Type["Engine"]] = None
    ) -> None:
        """
    ...
    def get_typed_engine(self, name: str, engine_type: Type[TEngine]) -> Optional[TEngine]:
        """
...
class HierarchicalStateSchema(BaseModel):
    ...
    agents: Dict[str, AgentState] = Field(default_factory=dict, description="Per-agent state")
    routing: RoutingState = Field(default_factory=RoutingState, description="Routing state")
...
def create_typed_state_schema(
    base_schema: Type[StateSchema],
    engine_type: Optional[Type[TEngine]] = None
) -> Type[StateSchema]:
    """
