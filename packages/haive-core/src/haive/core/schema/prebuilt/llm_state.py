...
class LLMState(MessagesStateWithTokenUsage):
    """State schema for LLM-based agents with single engine and token tracking.
    
    This schema is designed for agents that:
    - Use a single LLM engine (not multiple engines)
    - Need automatic token usage tracking for all messages
    - Want to compare token usage against engine metadata/thresholds
    - Require cost tracking and capacity monitoring
    
    The schema combines:
    - MessagesStateWithTokenUsage for automatic token tracking
    - Single engine field for cleaner LLM agent design
    ...
    engine: Engine = Field(
        ...,  # Required field
        description="The LLM engine for this agent"
    )
    
    # Hide engines field since we're single-engine focused
    engines: Dict[str, Engine] = Field(
        default_factory=dict,
        exclude=True,  # Exclude from serialization
        description="Hidden - use engine field instead"
    )
    ...
    context_length_override: Optional[int] = Field(
        default=None,
        description="Override auto-detected context length"
    )
    
    @model_validator(mode="after")
    def sync_engine_to_engines_dict(self) -> "LLMState":
        """Ensure the single engine is available in engines dict for compatibility."""
        if self.engine:
            # Add to engines dict under standard keys
    ...
    def remaining_tokens(self) -> int:
        ...
        elif hasattr(self.engine, "llm_config") and hasattr(self.engine.llm_config, "max_tokens"):
