...
def fix_rst_title_underlines():
    ...
    for rst_file in docs_dir.rglob("*.rst"):
        try:
            ...
            while i < len(lines):
                ...
                if i + 1 < len(lines):
                    ...
                    if next_line.strip() and all(c in '=-~^"+*#' for c in next_line.strip()):
                        ...
                        if current_underline_length != title_length and title_length > 0:
            ...
            logger.error(f"  ✗ Error fixing {rst_file}: {e}")
    ...
    try:
        ...
        if content != original_content:
            with open(showcase_file, 'w', encoding='utf-8') as f:
                f.write(content)
            logger.info(f"  ✓ Fixed showcase grid")
            return 1
            
    except Exception as e:
        logger.error(f"  ✗ Error fixing showcase: {e}")
    ...
    try:
        ...
        if content != original_content:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(content)
            logger.info(f"  ✓ Fixed docstrings in field_definition.py")
            return 1
...
def main():
    ...
    if total_fixes > 0:
        ...
        os.system("cd docs && poetry run sphinx-build -b html source build/html > /dev/null 2>&1")
        
        # Count remaining warnings
        result = os.popen("cd docs && poetry run sphinx-build -b html source build/html 2>&1 | grep -E 'WARNING|ERROR' | wc -l").read()
