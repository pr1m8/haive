    ...
    if hasattr(rag_agent.state_schema, 'model_fields'):
        ...
        for field_name in ['context', 'engine']:
            if field_name in fields:
                ...
                required = field_info.is_required() if hasattr(field_info, 'is_required') else (field_info.default is ...)
                print(f"   {field_name}: required={required}, default={getattr(field_info, 'default', 'NO_DEFAULT')}")
            else:
...
try:
    ...
    template = ChatPromptTemplate.from_messages([
        ...
        ("human", "Query: {query}\nContext: {context}")
    ]).partial(context="")  # This should make context optional
...
print("BaseRAGAgent likely doesn't have context/engine marked as required in its input_model,")
