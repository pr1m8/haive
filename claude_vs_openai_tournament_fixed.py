...
def create_claude_openai_config(game_config_class, claude_player: str, openai_player: str):
    ...
    if hasattr(config, 'engines') and isinstance(config.engines, dict):
        ...
        for engine_name, engine_config in engines.items():
            if claude_player in engine_name.lower() or 'player1' in engine_name.lower() or 'x' in engine_name.lower() or 'white' in engine_name.lower() or 'red' in engine_name.lower():
                # Assign Claude
                if hasattr(engine_config, 'llm_config') and hasattr(engine_config.llm_config, 'model'):
                    engine_config.llm_config.model = "claude-3-5-sonnet-20240620"
                if hasattr(engine_config, 'llm_config') and hasattr(engine_config.llm_config, 'provider'):
                    engine_config.llm_config.provider = "anthropic"
            elif openai_player in engine_name.lower() or 'player2' in engine_name.lower() or 'o' in engine_name.lower() or 'black' in engine_name.lower() or 'yellow' in engine_name.lower():
                # Assign OpenAI
                if hasattr(engine_config, 'llm_config') and hasattr(engine_config.llm_config, 'model'):
                    engine_config.llm_config.model = "gpt-4o"
                if hasattr(engine_config, 'llm_config') and hasattr(engine_config.llm_config, 'provider'):
                    engine_config.llm_config.provider = "openai"
        
        config.engines = engines
    
    return config

def run_tournament_game(game_name: str, config_class, agent_class, claude_player: str, openai_player: str):
    """Run a single tournament game."""
    matchup = f"claude_vs_openai"
    
    try:
        print(f"\n🎮 Tournament: {game_name} - Claude vs OpenAI")
        ...
        print(f"   Claude: {claude_player} | OpenAI: {openai_player}")
        ...
        try:
            ...
            if hasattr(result, 'winner') and result.winner:
                ...
                if 'X' in raw_winner or 'player1' in raw_winner or 'white' in raw_winner.lower() or 'red' in raw_winner.lower():
                    winner = "Claude"
                elif 'O' in raw_winner or 'player2' in raw_winner or 'black' in raw_winner.lower() or 'yellow' in raw_winner.lower():
                    winner = "OpenAI"
                else:
                    winner = f"Player: {raw_winner}"
            elif 'win' in final_status.lower():
                if 'player1' in final_status.lower() or 'X' in final_status or 'red' in final_status.lower():
                    winner = "Claude"
                elif 'player2' in final_status.lower() or 'O' in final_status or 'yellow' in final_status.lower():
                    winner = "OpenAI"
                else:
...
def main():
    ...
    print("🏟️ CLAUDE vs OPENAI TOURNAMENT - FIXED VERSION")
    print("================================================")
    ...
    for game in tournament_games:
        try:
            ...
            print(f"❌ Failed to run {game['name']}: {e}")
            tournament_results[game["name"]] = "Failed"
    
    # Final tournament summary
    print("\n" + "="*60)
    print("🏆 CLAUDE vs OPENAI TOURNAMENT RESULTS")
    print("="*60)
    ...
    for game, winner in tournament_results.items():
        ...
        print(f"{status_icon} {game.replace('_', ' ').title()}: {winner}")
    ...
    print(f"🚀 OpenAI: {openai_wins} wins") 
    print(f"🤝 Other/Draws: {other_results}")
    print(f"❌ Failed: {len([r for r in tournament_results.values() if r == 'Failed'])}")
    
    if claude_wins > openai_wins:
        print(f"\n🏆 TOURNAMENT CHAMPION: CLAUDE! ({claude_wins}-{openai_wins})")
    elif openai_wins > claude_wins:
        print(f"\n🏆 TOURNAMENT CHAMPION: OPENAI! ({openai_wins}-{claude_wins})")
    else:
        print(f"\n🤝 TOURNAMENT TIE! ({claude_wins}-{openai_wins})")
    
    print(f"\n📁 Detailed results: {TOURNAMENT_DIR.absolute()}")
