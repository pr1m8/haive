...
class MockValidationResult:
    def __init__(self, tool_call_id, tool_name, status, route_recommendation=None, 
                 errors=None, target_node=None, metadata=None, priority=0):
...
class SimpleStateUpdatingValidationNode:
    ...
    def create_node_function(self):
        ...
        def validation_node(state, config=None):
            ...
            print(f"[{self.name}] Validation complete: {summary['valid_count']} valid, {summary['error_count']} errors")
        ...
        def validation_router(state):
            ...
            if self.validation_mode == ValidationMode.STRICT:
                if routing_decision["error_count"] > 0 or routing_decision["invalid_count"] > 0:
                    print(f"[{self.name}] Strict mode: routing to agent due to failures")
                    return self.agent_node
            
            elif self.validation_mode == ValidationMode.PERMISSIVE:
                if routing_decision["valid_count"] == 0:
                    print(f"[{self.name}] Permissive mode: no valid tools, routing to agent")
                    return self.agent_node
...
def test_validation_node():
    ...
    if updated_state1.validation_state:
        ...
        print(f"     Valid: {summary1['valid_count']}, Errors: {summary1['error_count']}")
    ...
    if updated_state2.validation_state:
        ...
        print(f"     Valid: {summary2['valid_count']}, Errors: {summary2['error_count']}")
    ...
    if updated_state3.validation_state:
        ...
        print(f"     Valid: {summary3['valid_count']}, Errors: {summary3['error_count']}")
