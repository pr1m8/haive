"""D202 Fixer V3: Remove extra blank lines after function docstrings (corrected)."""
...
class D202FixerV3(BaseFixer):
    """Corrected fixer for D202: No blank lines allowed after function docstring.

    This version properly removes blank lines instead of just skipping them.
    """

    def __init__(self):
        """Initialize D202 fixer V3."""
        super().__init__(
            error_codes=["D202"],
            ...
            if line.strip().endswith('"""') or line.strip().endswith("'''"):
                ...
                if def_line_idx is not None:
                    ...
                    if blank_count > 0 and j < len(lines):
                        # Skip the blank lines by advancing i
                        i = j - 1  # Will be incremented at end of loop
                        fixes_count += 1
                        changes.append(
                            f"Removed {blank_count} blank line(s) after docstring at line {i + 1}"
                        )
                    # If no blank lines, just continue normally
            else:
                # Not a docstring end, just add the line
                fixed_lines.append(line)
