...
def debug_print_schema(schema):
    ...
    if hasattr(schema, 'model_fields'):
        for field_name, field_info in schema.model_fields.items():
            logger.debug(f"- {field_name}: Type={field_info.annotation}, Default={field_info.default}")
...
def test_simple_agent_schema_validation():
    ...
    logger.debug(f"Prompt Template Input Variables: {person_aug_llm.prompt_template.input_variables}")
    ...
    try:
        ...
        logger.error(f"Error deriving schemas: {e}")

    # Create a simple agent with the structured output model
    try:
        agent = create_simple_agent(
            engine=person_aug_llm,
            name="person_profile_agent",
            visualize=False
        )
    except Exception as e:
    ...
    assert model_name in state_schema.model_fields, f"Structured output field '{model_name}' should exist in schema"
    ...
    try:
        ...
        logger.error(f"Error creating state instance: {e}")
        raise
...
def test_agent_with_output_parsing():
    ...
    simple_prompt = ChatPromptTemplate.from_messages([
        ("system", "Summarize the input text concisely."),
        ("human", "{text}")
    ])
    ...
    try:
        ...
        logger.error(f"Error deriving input schema: {e}")
    
    # Create agent
    try:
        summary_agent = create_simple_agent(
            engine=summary_aug_llm,
            name="summary_agent",
            visualize=False
        )
    except Exception as e:
        logger.error(f"Error creating summary agent: {e}")
        logger.error(f"Full traceback: {traceback.format_exc()}")
        raise
    ...
    assert 'text' in summary_agent.state_schema.model_fields, "Input variable 'text' should be in schema"
    assert 'messages' in summary_agent.state_schema.model_fields, "Messages field should be in schema"
    assert 'runnable_config' in summary_agent.state_schema.model_fields, "runnable_config field should be in schema"
