...
class StateSchemaManagerTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.command_patcher = patch('src.haive.core.graph.schema.StateSchemaManager.Command', mock_command)
        self.command_patcher.start()
        
        self.send_patcher = patch('src.haive.core.graph.schema.StateSchemaManager.Send', mock_send)
    ...
    def test_init_with_dict(self):
        ...
        # Check name
        self.assertEqual(schema_manager.name, "TestDict")
        
        # Check fields
        self.assertIn("name", schema_manager.fields)
        self.assertIn("value", schema_manager.fields)
        self.assertIn("list_field", schema_manager.fields)
        self.assertIn("dict_field", schema_manager.fields)
        ...
        self.assertEqual(value_type, int)
    ...
    def test_init_with_model(self):
        ...
        with patch.object(StateSchemaManager, '_load_from_model', autospec=True) as mock_load:
            ...
            self.assertEqual(schema_manager.name, "TestPydanticModel")
            self.assertEqual(schema_manager.fields, fields)
            self.assertEqual(schema_manager.properties, properties)
            self.assertEqual(schema_manager.class_methods, class_methods)
            self.assertEqual(schema_manager.static_methods, static_methods)
    def test_add_field(self):
        ...
        schema_manager.add_field("config_field", str, default="config value", config_aware=True)
        
        # Check fields
        self.assertIn("simple_field", schema_manager.fields)
        self.assertIn("list_field", schema_manager.fields)
        self.assertIn("config_field", schema_manager.fields)
        
        # Check config awareness
        self.assertTrue(hasattr(schema_manager, "_config_aware_fields"))
        self.assertIn("config_field", schema_manager._config_aware_fields)
    ...
    def test_merge(self):
        ...
        # Check merged fields
        self.assertIn("field1", merged.fields)
        self.assertIn("field2", merged.fields)
        self.assertIn("common_field", merged.fields)
        
        # First occurrence should win for conflicts
        common_type, _ = merged.fields["common_field"]
        self.assertEqual(common_type, int)  # First type should be preserved
    ...
    def test_config_aware_fields(self):
        ...
        setattr(Model, "_config_aware_fields", {"config_field"})
        ...
        instance = Model()
        self.assertEqual(instance.config_field, "default")
        
        # Apply config
        instance.apply_config({"configurable": {"config_field": "configured_value"}})
        self.assertEqual(instance.config_field, "configured_value")
        
        # Normal field should not be changed
        self.assertEqual(instance.normal_field, "normal")
    ...
    def test_create_default_command(self):
        ...
        self.assertEqual(kwargs.get("update"), update)
        self.assertEqual(kwargs.get("goto"), goto)
        self.assertEqual(kwargs.get("resume"), {})
    ...
    def test_add_computed_property(self):
        ...
        self.assertIn("squared", schema.computed_properties)
        
        # Check getter and setter
        getter, setter = schema.computed_properties["squared"]
        self.assertEqual(getter, get_squared)
        self.assertEqual(setter, set_squared)
    ...
    def test_modify_and_remove_field(self):
        ...
        self.assertEqual(modified_type, int)
        
        # Remove field
        schema.remove_field("field2")
        
        # Check field removed
        self.assertNotIn("field2", schema.fields)
    ...
    def test_locked_schema(self):
        ...
        # Lock the schema
        model = schema.get_model(lock=True)
    ...
    def test_has_field(self):
        ...
        self.assertTrue(schema.has_field("existing_field"))
        self.assertFalse(schema.has_field("non_existing_field"))
