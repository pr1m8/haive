            ...
            if matches:
                if error_type == 'ModuleNotFoundError':
                    for missing_module in matches:
                        ...
                        if current_module:
                            errors_by_module[current_module].append(f"{error_type}: {missing_module}")
                            
                elif error_type == 'ImportError':
                    for missing_name, from_module in matches:
                        error_desc = f"cannot import '{missing_name}' from '{from_module}'"
                        errors_by_type[error_type].add(error_desc)
                        if current_module:
                            errors_by_module[current_module].append(f"{error_type}: {error_desc}")
                            
                elif error_type == 'TypeError_Abstract':
                    for class_name in matches:
                        errors_by_type[error_type].add(class_name)
                        if current_module:
                            errors_by_module[current_module].append(f"{error_type}: {class_name}")
                            
                elif error_type == 'NameError':
                    for undefined_name in matches:
                        errors_by_type[error_type].add(undefined_name)
                        if current_module:
                            errors_by_module[current_module].append(f"{error_type}: {undefined_name}")
                            
                else:  # Generic error types
                    errors_by_type[error_type].add(line.strip())
                    if current_module:
                        errors_by_module[current_module].append(f"{error_type}: {line.strip()}")
...
def categorize_errors(errors_by_type: Dict[str, Set[str]]) -> Dict[str, List[str]]:
    ...
    if 'TypeError_Abstract' in errors_by_type:
        categories['abstract_class_instantiation'].extend(errors_by_type['TypeError_Abstract'])
...
def generate_report(log_file: str, output_file: str = None):
    ...
    if 'abstract_class_instantiation' in categories:
        ignore_candidates.extend([f'"**/configurable_config.py"', f'"**/generic_engines.py"'])
        ...
        for error in categories['missing_imports_from_existing'][:10]:
            if "cannot import" in error:
                ...
                if len(parts) >= 4:
                    ...
                    from_module = parts[3]
                    report_lines.append(f"- Add `{missing_name}` to `{from_module}/__init__.py`")
...
if __name__ == "__main__":
    ...
    output_file = "/home/will/Projects/haive/backend/haive/comprehensive_import_error_analysis.md"
