...
log_progress "📝 Logging to: $LOG_FILE"
...
if [ "$1" == "--background" ]; then
	...
	PID=$!
	log_progress "📊 Build started with PID: $PID"
	log_progress "💡 Monitor progress with: tail -f $LOG_FILE"
	log_progress "💡 Check process with: ps aux | grep $PID"
else
	...
	if [ -f docs/build/html/index.html ]; then
		...
		log_progress "📋 Full log: $LOG_FILE"
	else
