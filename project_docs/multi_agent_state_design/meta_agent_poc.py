...
class MetaAgentState(StateSchema):
    ...
    def update_from_agent(
        ...
        updated_messages: Optional[List[BaseMessage]] = None
    ) -> None:
        """
...
class PlannerState(StateSchema):
    ...
    plan: Optional[str] = Field(default=None, description="Generated plan")
...
def test_meta_agent_state():
    ...
    print("=" * 60)
    print("TESTING META AGENT STATE PATTERN")
    print("=" * 60)
    ...
    print(f"\n✅ Registered agents: {list(meta_state.agent_schemas.keys())}")
    
    # Test 1: Get planner view
    print("\n--- Testing Planner View ---")
    planner_view = meta_state.get_agent_view("planner")
    
    print(f"Planner view type: {type(planner_view)}")
    print(f"Agent state type: {type(planner_view.agent_state)}")
    print(f"Messages: {len(planner_view.messages)}")
    print(f"Shared context: {planner_view.shared_context}")
    ...
    print(f"\n✅ Updated meta state after planner execution")
    print(f"Current agent: {meta_state.current_agent}")
    print(f"Execution history: {len(meta_state.execution_history)} entries")
    
    # Test 2: Get executor view
    print("\n--- Testing Executor View ---")
    ...
    print(f"Executor received plan: {executor_state.plan[:50]}...")
    ...
    print(f"Total agents: {len(meta_state.agent_schemas)}")
    print(f"Agent states stored: {len(meta_state.agent_states)}")
    print(f"Execution counts: {meta_state.execution_counts}")
    ...
    try:
        ...
        print("✅ Type safety maintained for PlannerState")
        ...
        print("✅ Agent-specific fields accessible")
        
    except Exception as e:
        print(f"❌ Type safety check failed: {e}")
...
def test_state_transitions():
    ...
    print("✅ State successfully transferred between agents")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("SUMMARY: Meta Agent State Pattern")
    print("=" * 60)
    print("✅ Type safety maintained through execution")
    print("✅ Agent states properly isolated")
    print("✅ Shared state accessible to all agents")
    print("✅ State transitions traceable")
    print("✅ Schema registration works dynamically")
    
    print("\nNext steps:")
    print("1. Integrate with AgentNodeV2")
    print("2. Add automatic state transfer rules")
    print("3. Implement routing based on state")
    print("4. Add validation at transition points")