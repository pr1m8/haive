...
class MarkdownConverterConfig(BaseModel):
    input_path: Path = Field(..., description="Directory containing markdown files or a single markdown file")
    output_dir: Optional[Path] = Field(default=None, description="Directory where converted files will be saved (defaults to 'rendered' folder in input directory)")
    formats: List[OutputFormat] = Field(default=[OutputFormat.HTML], description="Output formats to generate")
    ...
    markdown_extensions: List[str] = Field(default=["extra", "codehilite"], description="Markdown extensions to use")
    parallel: bool = Field(default=True, description="Whether to process files in parallel")
    max_workers: int = Field(default=4, description="Maximum number of parallel workers")
    pandoc_path: Optional[str] = Field(default=None, description="Path to pandoc executable")
    ...
    def validate_input_path_exists(cls, v):
        if not v.exists():
...
class MarkdownRenderer:
    def __init__(self, config: MarkdownConverterConfig):
        ...
        self.needs_pandoc = any(fmt in self.pandoc_required_formats for fmt in config.formats)
    ...
    def _check_pandoc(self):
        ...
        logger.info(f"Using pandoc: {result.stdout.splitlines()[0] if result.stdout else 'Unknown version'}")
    ...
    def save_as_html(self, md_file: MarkdownFile) -> Path:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("<style>body{font-family:system-ui,-apple-system,sans-serif;line-height:1.6;max-width:50em;margin:0 auto;padding:2em}</style>\n")
            f.write("</head>\n<body>\n")
    ...
    def save_with_pandoc(self, md_file: MarkdownFile, format: OutputFormat) -> Path:
        ...
        with tempfile.NamedTemporaryFile(suffix='.md', mode='w', encoding='utf-8', delete=False) as temp:
            temp_path = temp.name
            temp.write(content)
    ...
    def save_as_format(self, md_file: MarkdownFile, format: OutputFormat) -> Path:
        ...
        try:
            ...
            if isinstance(e, ConverterException):
                raise
            raise FormatConversionError(f"Failed to convert to {format}: {str(e)}")
...
class MarkdownConverter:
    ...
    def collect_markdown_files(self) -> List[MarkdownFile]:
        ...
        if input_path.is_file() and input_path.suffix.lower() in ['.md', '.markdown']:
            ...
            raise ValueError(f"Input path '{input_path}' is not a valid markdown file or directory")
    ...
    def _convert_file(self, md_file: MarkdownFile) -> List[Path]:
        ...
        for format in self.config.formats:
            try:
                ...
                logger.error(f"Error converting {md_file.path} to {format}: {e}")
                if isinstance(e, PandocMissingError):
                    # If pandoc is missing, skip all pandoc-required formats
                    logger.error("Skipping all pandoc-required formats due to missing pandoc")
                    break
        return output_paths
...
def convert_markdown(
    ...
    if not validated_formats:
        ...
        logger.info(f"No valid formats specified, defaulting to HTML")
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_path", help="Directory containing markdown files or a single markdown file")
    ...
    parser.add_argument("-e", "--extensions", nargs="+", default=["extra", "codehilite"],
