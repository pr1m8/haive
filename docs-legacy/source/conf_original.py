...
extensions = [
    ...
    # "sphinx-prompt",  # Optional - may not be installed
    ...
    "sphinx_pdj_theme",
    "sphinx_typo3_theme",
    # "groundwork_sphinx_theme",  # Optional - may not be installed
    ...
    "sphinx_autorun",
    "sphinx_click",
    # "sphinx_argparse",  # Optional - may not be installed
    # "sphinx_jsonschema",  # Optional - may not be installed
...
def check_and_remove_missing_extensions(extensions_list):
    ...
    if missing_extensions:
        # Only show missing extensions at DEBUG level to reduce noise
        logger.debug(f"⚠️ Missing {len(missing_extensions)} extensions: {', '.join(missing_extensions[:5])}...")
...
autoapi_options = [
    ...
    "undoc-members", 
    "show-inheritance",
    # "special-members",  # Disabled to reduce problematic member warnings  
    # "imported-members",  # Disabled to reduce noise from imported methods
]
...
# Sphinx Tippy configuration with safe fallback
if "sphinx_tippy" in extensions:
    try:
        # Log tippy setup for debugging
        logger.info("🔧 Setting up sphinx_tippy extension with enhanced safety")
        
        # DISABLE ALL tippy features to avoid KeyError(None) write_tippy_js issue
        tippy_enable_mathjax = False  # Avoid conflicts with other math processors
        tippy_enable_doitips = False  # Known to cause KeyError with None values - DISABLED
        tippy_enable_wikitips = False # Disable Wikipedia tooltips to avoid None key errors
        tippy_enable_rtdtips = False  # Disable RTD tooltips to avoid None key errors
        
        # Safe fallback configuration - minimal setup
        tippy_anchor_parent_selector = "main"  # Simple selector that exists in all themes
        
        # Empty configuration to prevent None key access
        tippy_rtd_urls = []  # Empty list to avoid None key issues
        tippy_custom_tips = {}  # Empty dict to initialize properly
        ...
        logger.info("✅ sphinx_tippy configured with all features DISABLED to prevent KeyError")
        logger.info("   🔒 doitips: OFF, wikitips: OFF, rtdtips: OFF (safety mode)")
        
    except Exception as e:
        logger.error(f"❌ sphinx_tippy configuration failed even with safety mode: {e}")
        logger.warning("🔄 Removing sphinx_tippy from extensions as ultimate fallback")
        try:
            extensions.remove("sphinx_tippy")
            logger.info("✅ sphinx_tippy safely removed from extensions")
        except ValueError:
            logger.debug("sphinx_tippy was already removed from extensions")
else:
...
def autoapi_skip_member(app, what, name, obj, skip, options):
    """Skip problematic members with enhanced logic to reduce warnings."""
    try:
        # Skip ALL methods that commonly cause "problematic member" warnings
        problematic_method_patterns = [
            # Pydantic V2 methods that cause issues
            "model_post_init", "model_validate", "model_dump", "model_copy",
            "model_json_schema", "model_fields_set", "model_extra",
            "__pydantic_", "model_", "_field_", "_schema_",
            
            # Class methods that cause issues
            "__class_getitem__", "__init_subclass__", "__new__",
            
            # Common inherited methods that spam warnings
            "copy", "dict", "json", "parse_obj", "parse_raw",
            "schema", "schema_json", "validate", "construct",
            
            # LangChain methods that cause issues
            "invoke", "ainvoke", "batch", "abatch", "stream", "astream",
            "transform", "atransform", "get_config", "apply_config",
            
            # BaseModel inherited methods
            "update_forward_refs", "parse_file", "from_orm",
            
            # Agent-specific methods that commonly fail
            "run", "arun", "execute", "process", "handle",
            "build", "create", "resolve", "derive", "extract",
            "get_", "set_", "add_", "remove_", "clear_", "update_",
            
            # Property and validation methods
            "property", "validator", "root_validator", "field_validator",
            
            # Game config methods that cause issues
            "ensure_engine", "use_pattern", "disable_pattern", "enable_pattern",
            "is_pattern_applied", "mark_pattern_applied", "with_config_overrides",
            "register_agent_class", "_generate_cache_key", "_generate_player_names",
            
            # Schema-related methods
            "input_schema", "output_schema", "state_schema", "node_configs",
            "_state_schema_instance", "_input_schema_instance", "_output_schema_instance",
        ]
        
        # Skip based on method name patterns
        if what == "method" and any(pattern in name for pattern in problematic_method_patterns):
            return True
        
        # Skip Pydantic internals and private attributes
        pydantic_internals = [
        ...
        if any(pattern in str(name) for pattern in problematic_patterns):
            # Don't log at WARNING level - too noisy. Use DEBUG instead
            logger.debug(f"Skipping problematic member: {name}")
            return True
