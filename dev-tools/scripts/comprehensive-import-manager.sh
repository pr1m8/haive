...
git stash push -m "IMPORT_MANAGER_CHECKPOINT_${TIMESTAMP}" -- "${DIRECTORY}" || echo "⚠️ No changes to stash"
...
echo "  🔴 Relative imports (should be absolute${: $RELATIVE_IMPO}RTS files"
echo "  🔴 Bad namespace imports (src.haive, packages.haive${: $BAD_NAMESPACE_IMPO}RTS files"
echo "  🔴 Undefined names (missing imports${: $UNDEFINED_NA}MES issues"
echo "  🔴 Unused import${: $UNUSED_IMPO}RTS issues"
echo "  📊 Total import-related issue${: $IMPORT_ERR}ORS"
echo ""
...
if [[ "${MODE}" = "--fix" ]]; then
	...
	echo "  🔴 Relative import${: $RELATIVE_IMPO}RTS${→ $FINAL_REL}ATIVE (fi${ed: $FIXED_REL}ATIVE)"
	echo "  🔴 Bad namespace import${: $BAD_NAMESPACE_IMPO}RTS${→ $FINAL_BAD_NAME}SPACE (fi${ed: $FIXED_BAD_NAME}SPACE)"
	echo "  📊 Total import issue${: $IMPORT_ERR}ORS${→ $FINAL_E}RRORS (fi${ed: $FIXED_}TOTAL)"
	echo ""
	...
	echo "  1. 🧹 Conve${t $RELATIVE_IMPO}RTS files with relative imports to absolute"
	echo "  2. 🧹 F${x $BAD_NAMESPACE_IMPO}RTS files with bad namespace patterns"
